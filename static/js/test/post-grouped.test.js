...
test('source exports PostGroupedPage as default class', () => {
  assert.ok(
    /export\s+default\s+class\s+PostGroupedPage/.test(source),
...
test('source imports TopicFlow', () => {
  assert.ok(
    /import\s+TopicFlow\s+from\s+['"]\.\/topic-flow\.js['"]/.test(source),
...
test('source imports TopicsRiverChart', () => {
  assert.ok(
    /import\s+TopicsRiverChart\s+from\s+['"]\.\/topics-river-chart\.js['"]/.test(source),
...
test('init calls addPostHoverEffects', () => {
  assert.ok(/this\.addPostHoverEffects\(\)/.test(source), 'init should call addPostHoverEffects');
});
...
test('init calls attachReadButtonHandlers', () => {
  assert.ok(
    /this\.attachReadButtonHandlers\(\)/.test(source),
    'init should call attachReadButtonHandlers'
  );
});

test('init calls initTabs', () => {
  assert.ok(/this\.initTabs\(\)/.test(source), 'init should call initTabs');
});

test('init calls initZoomControls', () => {
  assert.ok(/this\.initZoomControls\(\)/.test(source), 'init should call initZoomControls');
});

test('init calls handleHighlightSentenceFromUrl', () => {
  assert.ok(
    /this\.handleHighlightSentenceFromUrl\(\)/.test(source),
...
test('splitTopicPath trims whitespace around parts', () => {
  const page = new PostGroupedPage();
  assert.deepEqual(page.splitTopicPath('Science  >  Physics  >  Quantum'), [
    'Science',
    'Physics',
    'Quantum',
  ]);
...
test('colorFromString returns hsl format string', () => {
  ...
  assert.ok(/^hsl\(\d+, 60%, 60%\)$/.test(color), `expected hsl format, got: ${color}`);
});
...
test('getTopicLinks encodes topic path in href', () => {
  ...
  assert.ok(links[0].href.includes('topic=Science'), 'href should contain topic parameter');
});
...
test('setActiveTopic removes active class from .topic-tree-node.active', () => {
  assert.ok(
    /document\.querySelectorAll\s*\(\s*['"]\.topic-tree-node\.active['"]/.test(source),
    'should query .topic-tree-node.active'
  );
  assert.ok(/classList\.remove\s*\(\s*['"]active['"]/.test(source), 'should remove active class');
});

test('setActiveTopic uses topicElements Map to get element', () => {
  assert.ok(
    /this\.topicElements\.get\s*\(\s*topicPath/.test(source),
...
test('setActiveTopic adds active class to topicElement', () => {
  assert.ok(
    /topicElement\.classList\.add\s*\(\s*['"]active['"]/.test(source),
...
test('moveTopicPointer calls highlightSentences when has_grouped_data is true', () => {
  ...
  page.highlightSentences = (sentences, color, index) => {
    capturedArgs = { sentences, color, index };
  };
  globalThis.window = { has_grouped_data: true };
  page.moveTopicPointer('Science', 1);
...
test('moveTopicPointer calls highlightPosts when has_grouped_data is false', () => {
  ...
  let capturedArgs = null;
  page.topicState = { Science: { sentences: [10, 20, 30], index: 0, color: '#abc' } };
  page.highlightPosts = (sentenceIndices, color) => {
    capturedArgs = { sentenceIndices, color };
  };
...
test('moveTopicPointer returns early when sentences array is empty and no window.groups fallback', () => {
  ...
  page.highlightPosts = () => {
    highlightPostsCalled = true;
  };
  page.highlightSentences = () => {
    highlightSentencesCalled = true;
  };
  globalThis.window = { has_grouped_data: false, groups: {} };
  page.moveTopicPointer('Science', 1);
...
test('updateTopicReadButton toggles snippet-tag-read class', () => {
  ...
  const button = {
    ...
    classList: {
      toggle: (cls, state) => {
        toggles.push([cls, state]);
      },
    },
  };
  page.updateTopicReadButton(button, true);
  assert.deepEqual(
    toggles.find((t) => t[0] === 'snippet-tag-read'),
    ['snippet-tag-read', true]
  );
  assert.deepEqual(
    toggles.find((t) => t[0] === 'snippet-tag-unread'),
    ['snippet-tag-unread', false]
  );
});
...
test('changeSnippetsStatus calls fetch with /read/snippets endpoint', () => {
  assert.ok(/fetch\s*\(\s*['"]\/read\/snippets['"]/.test(source), 'should fetch /read/snippets');
});

test('changeSnippetsStatus sends POST request with JSON body', () => {
  assert.ok(/method\s*:\s*['"]POST['"]/.test(source), 'should use POST method');
  assert.ok(
    /'Content-Type'\s*:\s*['"]application\/json['"]/.test(source),
    'should set Content-Type header'
  );
  assert.ok(/JSON\.stringify/.test(source), 'should stringify body');
  assert.ok(/selections/.test(source), 'should include selections in body');
  assert.ok(/readed/.test(source), 'should include readed in body');
});
...
test('bindGlobalEvents checks for window.EVSYS', () => {
  assert.ok(/if\s*\(\s*window\.EVSYS\s*\)/.test(source), 'should check window.EVSYS');
});

test('bindGlobalEvents binds to POSTS_UPDATED event', () => {
  assert.ok(/window\.EVSYS\.POSTS_UPDATED/.test(source), 'should bind to POSTS_UPDATED');
});

test('bindGlobalEvents queries .post-read-status by data-post-id', () => {
  assert.ok(
    /\.post-read-status\[data-post-id/.test(source),
    'should query .post-read-status by data-post-id'
  );
});

test('bindGlobalEvents toggles read/unread classes on status button', () => {
  assert.ok(/classList\.remove\s*\(\s*['"]unread['"]/.test(source), 'should remove unread class');
  assert.ok(/classList\.add\s*\(\s*['"]read['"]/.test(source), 'should add read class');
});

test('bindGlobalEvents updates button textContent to read/unread', () => {
  assert.ok(/textContent\s*=\s*['"]read['"]/.test(source), 'should set textContent to "read"');
  assert.ok(/textContent\s*=\s*['"]unread['"]/.test(source), 'should set textContent to "unread"');
});

test('bindGlobalEvents updates section opacity based on read state', () => {
  assert.ok(
    /section\.style\.opacity\s*=\s*isRead\s*\?\s*['"]0\.6['"]/.test(source),
    'should set opacity to 0.6 when read'
  );
  assert.ok(/['"]1\.0['"]/.test(source), 'should set opacity to 1.0 when unread');
});
...
test('setInitialReadStatus queries .post-read-status.read', () => {
  assert.ok(
    /document\.querySelectorAll\s*\(\s*['"]\.post-read-status\.read['"]/.test(source),
...
test('setInitialReadStatus uses closest to find .post-section', () => {
  assert.ok(
    /btn\.closest\s*\(\s*['"]\.post-section['"]/.test(source),
    'should use closest(".post-section")'
  );
});

test('setInitialReadStatus sets opacity to 0.6 on read sections', () => {
  assert.ok(/section\.style\.opacity\s*=\s*['"]0\.6['"]/.test(source), 'should set opacity to 0.6');
});
...
test('attachReadButtonHandlers queries all .post-read-status', () => {
  assert.ok(
    /document\.querySelectorAll\s*\(\s*['"]\.post-read-status['"]/.test(source),
...
test('attachReadButtonHandlers calls event.stopPropagation', () => {
  assert.ok(/ev\.stopPropagation\(\)/.test(source), 'should call stopPropagation');
});

test('attachReadButtonHandlers reads data-post-id attribute', () => {
  assert.ok(/getAttribute\s*\(\s*['"]data-post-id['"]/.test(source), 'should read data-post-id');
});

test('attachReadButtonHandlers checks contains("read") class', () => {
  assert.ok(
    /classList\.contains\s*\(\s*['"]read['"]/.test(source),
...
test('changePostStatus fetches /read/posts endpoint', () => {
  assert.ok(/fetch\s*\(\s*['"]\/read\/posts['"]/.test(source), 'should fetch /read/posts');
});

test('changePostStatus sends ids array in body', () => {
  assert.ok(/ids\s*:\s*\[\s*postId/.test(source), 'should send ids array with postId');
});

test('changePostStatus sends readed boolean in body', () => {
  assert.ok(/readed\s*:\s*newStatus/.test(source), 'should send readed as newStatus');
});

test('changePostStatus checks response.ok', () => {
  assert.ok(/response\.ok/.test(source), 'should check response.ok');
});

test('changePostStatus logs error on catch', () => {
  assert.ok(
    /console\.error\s*\(\s*['"]Failed to update post status['"]/.test(source),
...
test('setupPostSections queries #grouped_posts .post-section', () => {
  assert.ok(
    /document\.querySelectorAll\s*\(\s*['"]#grouped_posts\s+\.post-section['"]/.test(source),
...
test('setupPostSections sets data-post-index from window.post_to_index_map', () => {
  assert.ok(/window\.post_to_index_map/.test(source), 'should use window.post_to_index_map');
  assert.ok(
    /setAttribute\s*\(\s*['"]data-post-index['"]/.test(source),
...
test('addPostHoverEffects adds mouseenter listener', () => {
  assert.ok(
    /addEventListener\s*\(\s*['"]mouseenter['"]/.test(source),
...
test('addPostHoverEffects adds mouseleave listener', () => {
  assert.ok(
    /addEventListener\s*\(\s*['"]mouseleave['"]/.test(source),
...
test('addPostHoverEffects sets boxShadow on hover', () => {
  assert.ok(
    /boxShadow\s*=\s*['"]0 2px 8px rgba\(66, 133, 244, 0\.2\)['"]/.test(source),
    'should set blue boxShadow'
  );
});

test('addPostHoverEffects resets boxShadow on mouseleave', () => {
  assert.ok(/boxShadow\s*=\s*['"]none['"]/.test(source), 'should reset boxShadow to none');
});
...
test('indexSentences clears sentencesByGlobalNumber', () => {
  assert.ok(
    /this\.sentencesByGlobalNumber\.clear\(\)/.test(source),
    'should call clear() on sentencesByGlobalNumber'
  );
});

test('indexSentences reads window.sentences', () => {
  assert.ok(/window\.sentences/.test(source), 'should read window.sentences');
});

test('indexSentences skips non-finite sentence numbers', () => {
  assert.ok(/Number\.isFinite\(globalNumber\)/.test(source), 'should check Number.isFinite');
});

test('indexSentences stores postId, postSentenceNumber, and read', () => {
  assert.ok(/postId\s*:\s*sentence\.post_id/.test(source), 'should store postId');
  assert.ok(/postSentenceNumber/.test(source), 'should store postSentenceNumber');
  assert.ok(/read\s*:\s*Boolean/.test(source), 'should store read as Boolean');
});
...
test('buildTopicsList queries #topics_list element', () => {
  assert.ok(
    /document\.getElementById\s*\(\s*['"]topics_list['"]/.test(source),
...
test('buildTopicsList returns early when topics_list not found', () => {
  assert.ok(
    /if\s*\(\s*!topicsList\s*\)/.test(source) || /if\s*\(!topicsList\)/.test(source),
...
test('buildTopicsList checks window.groups for empty state', () => {
  assert.ok(
    /Object\.keys\s*\(\s*window\.groups\s*\|\|\s*\{\}\s*\)/.test(source),
    'should check Object.keys(window.groups || {})'
  );
});

test('buildTopicsList renders "No topics available" message', () => {
  assert.ok(/No topics available/.test(source), 'should show "No topics available" message');
});
...
test('renderTopicNode creates topic-tree-node with depth class', () => {
  assert.ok(/topic-tree-node/.test(source), 'should use topic-tree-node class');
  assert.ok(/depth-/.test(source), 'should include depth class');
});

test('renderTopicNode sets --topic-accent CSS custom property', () => {
  assert.ok(/--topic-accent/.test(source), 'should set --topic-accent CSS property');
  assert.ok(/setProperty/.test(source), 'should use setProperty');
});

test('renderTopicNode creates topic-line, topic-title-wrap, topic-name, topic-count elements', () => {
  assert.ok(/topic-line/.test(source), 'should have topic-line class');
  assert.ok(/topic-title-wrap/.test(source), 'should have topic-title-wrap class');
  assert.ok(/topic-name/.test(source), 'should have topic-name class');
  assert.ok(/topic-count/.test(source), 'should have topic-count class');
});

test('renderTopicNode creates topic-links wrapper with topic-link class', () => {
  assert.ok(/topic-links/.test(source), 'should have topic-links class');
  assert.ok(/topic-link/.test(source), 'should use topic-link class');
});

test('renderTopicNode creates topic-controls with prev, next, read-toggle buttons', () => {
  assert.ok(/topic-controls/.test(source), 'should have topic-controls class');
  assert.ok(/topic-btn-prev/.test(source), 'should have topic-btn-prev class');
  assert.ok(/topic-btn-next/.test(source), 'should have topic-btn-next class');
  assert.ok(/topic-btn-read-toggle/.test(source), 'should have topic-btn-read-toggle class');
});

test('renderTopicNode stores element in topicElements Map', () => {
  assert.ok(
    /this\.topicElements\.set\s*\(\s*topicPath/.test(source),
...
test('renderTopicNode adds click handler on topic-line', () => {
  assert.ok(
    /line\.addEventListener\s*\(\s*['"]click['"]/.test(source),
    'should add click listener on topic-line'
  );
});

test('renderTopicNode checks isContentReady before handling topic click', () => {
  assert.ok(/!this\.isContentReady/.test(source), 'should check isContentReady');
});

test('renderTopicNode prevents prev/next button events from propagating', () => {
  assert.ok(/prevBtn\.addEventListener/.test(source), 'should add prev button listener');
  assert.ok(/ev\.preventDefault\(\)/.test(source), 'should call preventDefault');
});

test('renderTopicNode renders children with topic-children wrapper', () => {
  assert.ok(/topic-children/.test(source), 'should have topic-children class');
});
...
test('attachSentenceGroupHandlers queries .sentence-group', () => {
  assert.ok(
    /document\.querySelectorAll\s*\(\s*['"]\.sentence-group['"]/.test(source),
    'should query .sentence-group'
  );
});

test('attachSentenceGroupHandlers parses data-sentence as integer', () => {
  assert.ok(/parseInt/.test(source), 'should use parseInt');
  assert.ok(
    /getAttribute\s*\(\s*['"]data-sentence['"]/.test(source),
    'should read data-sentence attribute'
  );
});

test('attachSentenceGroupHandlers searches window.groups for topic', () => {
  assert.ok(/window\.groups/.test(source), 'should search window.groups');
  assert.ok(
    /includes\s*\(\s*sentNum/.test(source) || /\.includes\(/.test(source),
    'should check if group includes sentence'
  );
});

test('attachSentenceGroupHandlers clicks topic-line when found', () => {
  assert.ok(/topicLine\.click\(\)/.test(source), 'should click topic-line');
});
...
test('highlightSentences returns early for empty indices', () => {
  assert.ok(
    /!sentenceIndices\s*\|\|\s*sentenceIndices\.length\s*===\s*0/.test(source),
...
test('highlightSentences removes highlighted class from previous highlights', () => {
  assert.ok(
    /\.sentence-group\.highlighted/.test(source),
    'should query .sentence-group.highlighted'
  );
  assert.ok(
    /classList\.remove\s*\(\s*['"]highlighted['"]/.test(source),
...
test('highlightSentences adds highlighted class', () => {
  assert.ok(
    /classList\.add\s*\(\s*['"]highlighted['"]/.test(source),
    'should add highlighted class'
  );
});

test('highlightSentences scrolls focused sentence into view', () => {
  assert.ok(/scrollIntoView/.test(source), 'should call scrollIntoView');
  assert.ok(/behavior\s*:\s*['"]smooth['"]/.test(source), 'should use smooth scroll behavior');
  assert.ok(/block\s*:\s*['"]center['"]/.test(source), 'should scroll to center');
});

test('highlightSentences adds and removes pulse class', () => {
  assert.ok(/classList\.add\s*\(\s*['"]pulse['"]/.test(source), 'should add pulse class');
  assert.ok(/classList\.remove\s*\(\s*['"]pulse['"]/.test(source), 'should remove pulse class');
});

test('highlightSentences uses setTimeout with 100ms delay for scroll', () => {
  assert.ok(/setTimeout[\s\S]*100/.test(source), 'should use 100ms delay');
});

test('highlightSentences uses setTimeout with 1200ms for pulse removal', () => {
  assert.ok(/1200/.test(source), 'should use 1200ms for pulse removal');
});
...
test('highlightPosts returns early for empty postIds', () => {
  assert.ok(
    /!postIds\s*\|\|\s*postIds\.length\s*===\s*0/.test(source),
...
test('highlightPosts queries #grouped_posts .post-section', () => {
  assert.ok(
    /#grouped_posts\s+\.post-section/.test(source),
    'should query #grouped_posts .post-section'
  );
});
...
test('highlightPosts removes range-highlight and resets styles', () => {
  assert.ok(
    /classList\.remove\s*\(\s*['"]range-highlight['"]/.test(source),
    'should remove range-highlight'
  );
  assert.ok(/backgroundColor\s*=\s*['"]['"]/.test(source), 'should reset background color');
});

test('highlightPosts uses color with 40 hex alpha suffix', () => {
  assert.ok(/color\s*\+\s*['"]40['"]/.test(source), 'should append 40 alpha to color');
});

test('highlightPosts sets 3px solid border-left with color', () => {
  assert.ok(
    /borderLeft\s*=\s*['"]3px solid ['"]\s*\+\s*color/.test(source),
...
test('handleHighlightSentenceFromUrl reads highlight_sentence from URL search params', () => {
  assert.ok(/URLSearchParams/.test(source), 'should use URLSearchParams');
  assert.ok(/window\.location\.search/.test(source), 'should read window.location.search');
  assert.ok(
    /get\s*\(\s*['"]highlight_sentence['"]/.test(source),
    'should get highlight_sentence param'
  );
});

test('handleHighlightSentenceFromUrl returns early if param is NaN', () => {
  assert.ok(/isNaN\s*\(\s*highlightSentNum\s*\)/.test(source), 'should check isNaN');
});

test('handleHighlightSentenceFromUrl searches window.groups for topic', () => {
  assert.ok(/Object\.keys\s*\(\s*window\.groups/.test(source), 'should iterate window.groups keys');
});
...
test('handleHighlightSentenceFromUrl uses 500ms delay for highlight', () => {
  assert.ok(
    /setTimeout[\s\S]*500/.test(source) || /500\s*\)/.test(source),
...
test('handleHighlightSentenceFromUrl scrolls topic into view', () => {
  assert.ok(
    /scrollIntoView\s*\(\s*\{[^}]*behavior\s*:\s*['"]smooth['"]/.test(source),
...
test('initTabs queries .tab-header > .tab-btn', () => {
  assert.ok(
    /document\.querySelectorAll\s*\(\s*['"]\.tab-header\s*>\s*\.tab-btn['"]/.test(source),
...
test('initTabs queries .tab-content', () => {
  assert.ok(
    /document\.querySelectorAll\s*\(\s*['"]\.tab-content['"]/.test(source),
...
test('initTabs returns early if tabs or contents are empty', () => {
  assert.ok(
    /!tabs\.length\s*\|\|\s*!contents\.length/.test(source),
    'should check for empty tabs/contents'
  );
});

test('initTabs reads data-tab attribute', () => {
  assert.ok(/getAttribute\s*\(\s*['"]data-tab['"]/.test(source), 'should read data-tab attribute');
});

test('initTabs removes active class from all tabs and contents', () => {
  assert.ok(
    /t\.classList\.remove\s*\(\s*['"]active['"]/.test(source),
    'should remove active from tabs'
  );
  assert.ok(
    /c\.classList\.remove\s*\(\s*['"]active['"]/.test(source),
    'should remove active from contents'
  );
});

test('initTabs activates tab-posts for "posts" tab', () => {
  assert.ok(/['"]tab-posts['"]/.test(source), 'should reference tab-posts');
});

test('initTabs activates tab-topic-chart for "topic-chart" tab', () => {
  assert.ok(/['"]tab-topic-chart['"]/.test(source), 'should reference tab-topic-chart');
});

test('initTabs calls initChart when topic-chart tab is activated first time', () => {
  assert.ok(/!this\.chartInitialized/.test(source), 'should check chartInitialized flag');
  assert.ok(/this\.topicFlowChart\s*=\s*this\.initChart\(\)/.test(source), 'should call initChart');
  assert.ok(
    /this\.chartInitialized\s*=\s*true/.test(source),
    'should set chartInitialized to true'
  );
});

test('initTabs calls initLocalTabs', () => {
  assert.ok(/this\.initLocalTabs\(\)/.test(source), 'should call initLocalTabs');
});
...
test('initLocalTabs queries .local-tab-btn', () => {
  assert.ok(
    /document\.querySelectorAll\s*\(\s*['"]\.local-tab-btn['"]/.test(source),
...
test('initLocalTabs reads data-post-id and data-local-tab attributes', () => {
  assert.ok(/getAttribute\s*\(\s*['"]data-post-id['"]/.test(source), 'should read data-post-id');
  assert.ok(
    /getAttribute\s*\(\s*['"]data-local-tab['"]/.test(source),
...
test('initLocalTabs queries #post_ + postId', () => {
  assert.ok(
    /getElementById\s*\(\s*['"]post_['"]\s*\+\s*postId/.test(source),
...
test('initLocalTabs toggles active class on local tab buttons', () => {
  assert.ok(
    /\.local-tab-btn['"]\)\.forEach.*classList\.remove\s*\(\s*['"]active['"]/.test(source),
    'should remove active from sibling tabs'
  );
  assert.ok(
    /tab\.classList\.add\s*\(\s*['"]active['"]/.test(source),
    'should add active to clicked tab'
  );
});

test('initLocalTabs toggles active class on local tab content', () => {
  assert.ok(/\.local-tab-content/.test(source), 'should query .local-tab-content');
  assert.ok(/data-local-content/.test(source), 'should use data-local-content');
});

test('initLocalTabs initializes river chart when target is "river"', () => {
  assert.ok(/target\s*===\s*['"]river['"]/.test(source), 'should check for river target');
  assert.ok(/this\.initRiverChart\(postId\)/.test(source), 'should call initRiverChart');
  assert.ok(
    /setTimeout[\s\S]*10/.test(source) || /,\s*10\s*\)/.test(source),
...
test('initRiverChart caches chart instance in riverCharts', () => {
  assert.ok(/this\.riverCharts\[postId\]/.test(source), 'should use riverCharts[postId]');
});

test('initRiverChart calls render on cached chart', () => {
  assert.ok(
    /this\.riverCharts\[postId\]\.render\(\)/.test(source),
    'should call render on cached chart'
  );
});

test('initRiverChart finds post from window.posts', () => {
  assert.ok(/window\.posts\.find/.test(source), 'should use window.posts.find');
});

test('initRiverChart checks post.river_data', () => {
  assert.ok(/!post\s*\|\|\s*!post\.river_data/.test(source), 'should check post and river_data');
});

test('initRiverChart creates container ID as river_chart_ + postId', () => {
  assert.ok(/['"]river_chart_['"]\s*\+\s*postId/.test(source), 'should use river_chart_ + postId');
});

test('initRiverChart instantiates TopicsRiverChart with topics and articleLength', () => {
  assert.ok(/new\s+TopicsRiverChart/.test(source), 'should instantiate TopicsRiverChart');
  assert.ok(/river_data\.topics/.test(source), 'should pass river_data.topics');
  assert.ok(/river_data\.articleLength/.test(source), 'should pass river_data.articleLength');
});
...
test('initZoomControls queries zoom-in, zoom-out, reset-zoom by ID', () => {
  assert.ok(/getElementById\s*\(\s*['"]zoom-in['"]/.test(source), 'should get zoom-in');
  assert.ok(/getElementById\s*\(\s*['"]zoom-out['"]/.test(source), 'should get zoom-out');
  assert.ok(/getElementById\s*\(\s*['"]reset-zoom['"]/.test(source), 'should get reset-zoom');
});

test('initZoomControls returns early if any zoom element is missing', () => {
  assert.ok(
    /!zoomIn\s*\|\|\s*!zoomOut\s*\|\|\s*!resetZoom/.test(source),
...
test('initZoomControls adds click handlers to zoom buttons', () => {
  assert.ok(
    /zoomIn\.addEventListener\s*\(\s*['"]click['"]/.test(source),
    'should add click to zoomIn'
  );
  assert.ok(
    /zoomOut\.addEventListener\s*\(\s*['"]click['"]/.test(source),
    'should add click to zoomOut'
  );
  assert.ok(
    /resetZoom\.addEventListener\s*\(\s*['"]click['"]/.test(source),
    'should add click to resetZoom'
  );
});

test('initZoomControls calls zoomIn, zoomOut, resetZoom on topicFlowChart', () => {
  assert.ok(/this\.topicFlowChart\.zoomIn\(\)/.test(source), 'should call zoomIn');
  assert.ok(/this\.topicFlowChart\.zoomOut\(\)/.test(source), 'should call zoomOut');
  assert.ok(/this\.topicFlowChart\.resetZoom\(\)/.test(source), 'should call resetZoom');
});

test('initZoomControls guards against null topicFlowChart', () => {
  assert.ok(
    /if\s*\(\s*this\.topicFlowChart\s*\)/.test(source),
...
test('initChart returns null when window.groups is missing', () => {
  assert.ok(
    /if\s*\(\s*!window\.groups\s*\)\s*return\s+null/.test(source),
...
test('initChart maps window.groups to children array with name and value', () => {
  assert.ok(
    /Object\.keys\s*\(\s*window\.groups\s*\)\.map/.test(source),
    'should map Object.keys(window.groups)'
  );
  assert.ok(/name\s*:\s*key/.test(source), 'should set name from key');
  assert.ok(
    /value\s*:\s*window\.groups\[key\]\.length/.test(source),
    'should set value from group length'
  );
});

test('initChart creates data object with name "Topics"', () => {
  assert.ok(/name\s*:\s*['"]Topics['"]/.test(source), 'should set name to "Topics"');
});

test('initChart instantiates TopicFlow with data and #topic_flow_chart selector', () => {
  assert.ok(
    /new\s+TopicFlow\s*\(\s*data\s*,\s*['"]#topic_flow_chart['"]/.test(source),
...
test('setReadStateForSelections matches postId and post_sentence_number', () => {
  assert.ok(
    /String\s*\(\s*sentence\.post_id\s*\)\s*!==\s*postId/.test(source),
    'should compare post_id'
  );
  assert.ok(/sentence\.post_sentence_number/.test(source), 'should check post_sentence_number');
});

test('setReadStateForSelections updates sentence.read flag', () => {
  assert.ok(/sentence\.read\s*=\s*readed/.test(source), 'should update sentence.read');
});
...
test('setReadStateForSelections toggles sentence-read class on .sentence-group elements', () => {
  ...
  assert.ok(
    /classList\.toggle\s*\(\s*['"]sentence-read['"]/.test(source),
...
test('refreshTopicReadButtons queries .topic-btn-read-toggle', () => {
  assert.ok(
    /document\.querySelectorAll\s*\(\s*['"]\.topic-btn-read-toggle['"]/.test(source),
    'should query .topic-btn-read-toggle'
  );
});

test('refreshTopicReadButtons reads dataset.topicName', () => {
  assert.ok(/button\.dataset\.topicName/.test(source), 'should read dataset.topicName');
});

test('refreshTopicReadButtons calls updateTopicReadButton with isTopicFullyRead', () => {
  assert.ok(
    /this\.updateTopicReadButton\s*\(\s*button\s*,\s*this\.isTopicFullyRead/.test(source),
...
test('buildPostsList queries #posts_list element', () => {
  assert.ok(
    /document\.getElementById\s*\(\s*['"]posts_list['"]/.test(source),
...
test('buildPostsList returns early if posts_list missing or posts <= 1', () => {
  assert.ok(
    /!postsList\s*\|\|\s*!window\.posts\s*\|\|\s*window\.posts\.length\s*<=\s*1/.test(source),
...
test('buildPostsList creates topic-item elements with blue styling', () => {
  ...
  assert.ok(/'#4285f440'/.test(source), 'should set blue background color');
  assert.ok(
    /borderLeft\s*=\s*['"]4px solid #4285f4['"]/.test(source),
    'should set blue border-left'
  );
});

test('buildPostsList shows post_id and feed_title', () => {
  assert.ok(/post\.post_id/.test(source), 'should display post_id');
  assert.ok(/post\.feed_title/.test(source), 'should display feed_title');
...
test('buildPostsList adds range-highlight and pulse classes on click', () => {
  assert.ok(
    /classList\.add\s*\(\s*['"]range-highlight['"]\s*,\s*['"]pulse['"]/.test(source),
    'should add range-highlight and pulse'
  );
});

test('buildPostsList removes highlight after 2000ms', () => {
  assert.ok(/2000/.test(source), 'should use 2000ms timeout');
});
...
test('stripGlobalStyles queries .post-text containers', () => {
  assert.ok(
    /document\.querySelectorAll\s*\(\s*['"]\.post-text['"]/.test(source),
...
test('stripGlobalStyles removes style elements', () => {
  assert.ok(
    /container\.querySelectorAll\s*\(\s*['"]style['"]/.test(source),
    'should query style elements'
  );
  assert.ok(/styleNode\.remove\(\)/.test(source), 'should call remove() on style nodes');
});

test('stripGlobalStyles removes link[rel="stylesheet"] elements', () => {
  assert.ok(/link\[rel="stylesheet"\]/.test(source), 'should query link[rel="stylesheet"]');
  assert.ok(/linkNode\.remove\(\)/.test(source), 'should call remove() on link nodes');
});
