...
function createTagsClustersEventSystem() {
...
test('constructor sets default state snapshot', () => {
  ...
  const storage = new TagsClustersStorage(createTagsClustersEventSystem());
...
test('start() binds CHANGE_TAGS_LOAD_BUTTON_STATE', () => {
  global.document = { location: { hash: '' } };
  const es = createTagsClustersEventSystem();
  const storage = new TagsClustersStorage(es);
...
test('fetchTagClusters() success updates state and emits payload shape', async (t) => {
  ...
  rsstag_utils.fetchJSON = async () => ({ data: { c1: ['a', 'b'] } });
  t.after(() => {
