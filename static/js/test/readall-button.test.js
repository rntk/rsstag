...
test('class name is ReadAllButton', () => {
  const src = readSource();
  assert.ok(/export default class ReadAllButton/.test(src),
    'should define class ReadAllButton');
});
...
test('constructor initializes state with ids array and readed boolean', () => {
  ...
  assert.ok(/this\.state\s*=\s*\{[\s\S]*?readed\s*:\s*false/.test(src),
    'should set state.readed to false');
});

test('constructor binds changePostsStatus method', () => {
  const src = readSource();
  assert.ok(/this\.changePostsStatus\s*=\s*this\.changePostsStatus\.bind\(this\)/.test(src),
...
test('updatePosts sets readed from state.readed', () => {
  const src = readSource();
  assert.ok(/readed\s*:\s*state\.readed/.test(src),
    'should set readed from state.readed');
});
...
test('source declares changePostsStatus method', () => {
  const src = readSource();
  assert.ok(/changePostsStatus\s*\(\s*e\s*\)\s*\{/.test(src),
...
test('changePostsStatus creates data with ids copy and toggled readed', () => {
  ...
  assert.ok(/readed\s*:\s*!\s*this\.state\.readed/.test(src),
    'should toggle readed state');
});

test('changePostsStatus triggers CHANGE_POSTS_STATUS event', () => {
  const src = readSource();
  assert.ok(/this\.props\.ES\.trigger\s*\(\s*this\.props\.ES\.CHANGE_POSTS_STATUS/.test(src),
...
test('render binds onClick to changePostsStatus', () => {
  const src = readSource();
  assert.ok(/onClick\s*=\s*\{?\s*this\.changePostsStatus/.test(src),
    'should bind onClick to changePostsStatus');
});

test('render conditionally shows unread/read text', () => {
  const src = readSource();
  assert.ok(/this\.state\.readed\s*\?\s*['"]unread['"]/.test(src),
    'should show "unread" when readed is true');
  assert.ok(/:\s*['"]read['"]/.test(src),
    'should show "read" when readed is false');
});
