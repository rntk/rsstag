...
test('class name is GlobalStatus', () => {
  const src = readSource();
  assert.ok(/export default class GlobalStatus/.test(src), 'should define class GlobalStatus');
});
...
test('constructor initializes state with msgs, is_ok, promptType, promptValue', () => {
  const src = readSource();
  assert.ok(/this\.state\s*=\s*\{[\s\S]*?msgs\s*:/.test(src), 'should set state.msgs');
  assert.ok(/this\.state\s*=\s*\{[\s\S]*?is_ok\s*:/.test(src), 'should set state.is_ok');
  assert.ok(/this\.state\s*=\s*\{[\s\S]*?promptType\s*:/.test(src), 'should set state.promptType');
  assert.ok(
    /this\.state\s*=\s*\{[\s\S]*?promptValue\s*:/.test(src),
...
test('constructor stores ES reference from props', () => {
  const src = readSource();
  assert.ok(/this\.ES\s*=\s*props\.ES/.test(src), 'should store this.ES = props.ES');
});

test('constructor initializes timeout_handler to 0', () => {
  const src = readSource();
  assert.ok(/this\.timeout_handler\s*=\s*0/.test(src), 'should initialize timeout_handler = 0');
});

test('constructor creates promptInputRef with React.createRef', () => {
  const src = readSource();
  assert.ok(
    /this\.promptInputRef\s*=\s*React\.createRef/.test(src),
...
test('constructor binds immediatlyCheck method', () => {
  const src = readSource();
  assert.ok(
    /this\.immediatlyCheck\s*=\s*this\.checkStatusAfter\.bind/.test(src),
...
test('source declares checkStatusAfter method', () => {
  const src = readSource();
  assert.ok(/checkStatusAfter\s*\(/.test(src), 'should declare checkStatusAfter() method');
});

test('checkStatusAfter uses setTimeout for scheduling', () => {
  const src = readSource();
  assert.ok(/setTimeout/.test(src), 'should use setTimeout');
});

test('checkStatusAfter clears previous timeout', () => {
  const src = readSource();
  assert.ok(/clearTimeout/.test(src), 'should call clearTimeout before setting new timeout');
});

test('checkStatusAfter calls fetchStatus', () => {
  const src = readSource();
  assert.ok(/this\.fetchStatus/.test(src), 'should call fetchStatus in timeout callback');
});

test('source declares fetchStatus method', () => {
  const src = readSource();
  assert.ok(/fetchStatus\s*\(\s*\)/.test(src), 'should declare fetchStatus() method');
});

test('fetchStatus calls /status endpoint', () => {
  const src = readSource();
  assert.ok(/['"]\/status['"]/.test(src), 'should fetch from /status endpoint');
});
...
test('fetchStatus calls checkStatusAfter with 60000ms interval', () => {
  const src = readSource();
  assert.ok(/60000/.test(src), 'should schedule next check after 60 seconds');
});

test('source declares openTelegramPrompt method', () => {
  const src = readSource();
  assert.ok(/openTelegramPrompt\s*\(/.test(src), 'should declare openTelegramPrompt() method');
});

test('source declares handlePromptChange method', () => {
  const src = readSource();
  assert.ok(/handlePromptChange\s*=/.test(src), 'should declare handlePromptChange arrow function');
});

test('source declares handlePromptSubmit method', () => {
  const src = readSource();
  assert.ok(/handlePromptSubmit\s*=/.test(src), 'should declare handlePromptSubmit arrow function');
});

test('handlePromptSubmit calls preventDefault', () => {
  const src = readSource();
  assert.ok(/preventDefault/.test(src), 'should call preventDefault on event');
});

test('handlePromptSubmit triggers SAVE_TELEGRAM_PASSWORD', () => {
  const src = readSource();
  assert.ok(/SAVE_TELEGRAM_PASSWORD/.test(src), 'should trigger SAVE_TELEGRAM_PASSWORD event');
});

test('handlePromptSubmit triggers SAVE_TELEGRAM_CODE', () => {
  const src = readSource();
  assert.ok(/SAVE_TELEGRAM_CODE/.test(src), 'should trigger SAVE_TELEGRAM_CODE event');
});
...
test('componentDidMount schedules initial status check', () => {
  const src = readSource();
  assert.ok(
    /componentDidMount[\s\S]*?checkStatusAfter/.test(src),
...
test('componentDidUpdate focuses input on promptType change', () => {
  const src = readSource();
  assert.ok(/\.focus\s*\(\s*\)/.test(src), 'should call .focus() on input');
  assert.ok(/promptInputRef\.current/.test(src), 'should use promptInputRef.current for focus');
});
...
test('render returns null when state is null', () => {
  const src = readSource();
  assert.ok(
    /if\s*\(\s*!\s*this\.state\s*\)\s*\{[\s\S]*?return null/.test(src),
...
test('render uses React.Fragment', () => {
  const src = readSource();
  assert.ok(/React\.Fragment/.test(src), 'should use React.Fragment wrapper');
});

test('render shows error link with error CSS class', () => {
  const src = readSource();
  assert.ok(/className.*['"]error['"]/.test(src), 'should use error CSS class on error link');
});

test('render error link points to /provider', () => {
  const src = readSource();
  assert.ok(/['"]\/provider['"]/.test(src), 'should link to /provider on error');
});

test('render uses abbr element for status display', () => {
  const src = readSource();
  assert.ok(/<abbr/.test(src), 'should use <abbr> element for status');
});

test('render shows ERROR in title attribute', () => {
  const src = readSource();
  assert.ok(/ERROR/.test(src), 'should include ERROR in title');
});

test('render shows Working in title attribute', () => {
  const src = readSource();
  assert.ok(/Working/.test(src), 'should include Working in title for active tasks');
});

test('render shows No active tasks in title', () => {
  const src = readSource();
  assert.ok(/No active tasks/.test(src), 'should include "No active tasks" in title');
});
...
test('render calls immediatlyCheck on status click', () => {
  const src = readSource();
  assert.ok(
    /onClick\s*=\s*\{?\s*this\.immediatlyCheck/.test(src),
...
test('render includes telegram-auth-overlay CSS class', () => {
  const src = readSource();
  assert.ok(/telegram-auth-overlay/.test(src), 'should use telegram-auth-overlay CSS class');
});

test('render includes telegram-auth-modal CSS class', () => {
  const src = readSource();
  assert.ok(/telegram-auth-modal/.test(src), 'should use telegram-auth-modal CSS class');
});

test('render includes telegram-auth-label CSS class', () => {
  const src = readSource();
  assert.ok(/telegram-auth-label/.test(src), 'should use telegram-auth-label CSS class');
});

test('render includes telegram-auth-input CSS class', () => {
  const src = readSource();
  assert.ok(/telegram-auth-input/.test(src), 'should use telegram-auth-input CSS class');
});

test('render includes telegram-auth-submit CSS class', () => {
  const src = readSource();
  assert.ok(/telegram-auth-submit/.test(src), 'should use telegram-auth-submit CSS class');
});

test('modal form has onSubmit handler', () => {
  const src = readSource();
  assert.ok(
    /onSubmit\s*=\s*\{?\s*this\.handlePromptSubmit/.test(src),
...
test('input uses onChange to handlePromptChange', () => {
  const src = readSource();
  assert.ok(
    /onChange\s*=\s*\{?\s*this\.handlePromptChange/.test(src),
...
test('input has required attribute', () => {
  const src = readSource();
  assert.ok(/required/.test(src), 'should have required on input');
});

test('modal uses password input type for sensitive prompts', () => {
  const src = readSource();
  assert.ok(/password/.test(src), 'should use type="password" for telegram prompts');
});

test('modal uses autoComplete for one-time-code or current-password', () => {
  const src = readSource();
  assert.ok(/current-password/.test(src), 'should use current-password autoComplete');
  assert.ok(/one-time-code/.test(src), 'should use one-time-code autoComplete');
});

test('modal shows Telegram password label', () => {
  const src = readSource();
  assert.ok(/Telegram password/.test(src), 'should include "Telegram password" label');
});

test('modal shows Telegram code label', () => {
  const src = readSource();
  assert.ok(/Telegram code/.test(src), 'should include "Telegram code" label');
});

test('modal renders conditionally based on promptType', () => {
  const src = readSource();
  assert.ok(
    /this\.state\.promptType &&/.test(src),
...
test('fetchStatus checks data.data.is_ok', () => {
  const src = readSource();
  assert.ok(/data\.data\.is_ok/.test(src), 'should check data.data.is_ok');
});

test('fetchStatus checks data.data.msgs', () => {
  const src = readSource();
  assert.ok(/data\.data\.msgs/.test(src), 'should check data.data.msgs');
});

test('fetchStatus checks telegram_password flag', () => {
  const src = readSource();
  assert.ok(/telegram_password/.test(src), 'should check telegram_password flag');
});

test('fetchStatus checks telegram_code flag', () => {
  const src = readSource();
  assert.ok(/telegram_code/.test(src), 'should check telegram_code flag');
});
...
test('source imports rsstag_utils', () => {
  const src = readSource();
  assert.ok(/import.*rsstag_utils/.test(src), 'should import rsstag_utils');
});

test('source uses rsstag_utils.fetchJSON', () => {
  const src = readSource();
  assert.ok(/rsstag_utils\s*\.\s*fetchJSON/.test(src), 'should use rsstag_utils.fetchJSON');
});
