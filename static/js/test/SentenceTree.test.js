...
const source = fs.readFileSync(
  new URL('../components/SentenceTree.js', import.meta.url),
  'utf8'
);
...
test('SentenceTree uses strict mode', () => {
  assert.ok(
    /'use strict'/.test(source),
    'source should start with use strict'
  );
});
...
test('SentenceTree constructor reads from props.s_tree_data', () => {
  assert.ok(
    /props\.s_tree_data/.test(source),
    'should read from props.s_tree_data'
  );
});

test('SentenceTree constructor reads from window.s_tree_data', () => {
  assert.ok(
    /window\.s_tree_data/.test(source),
    'should read from window.s_tree_data'
  );
});
...
test('SentenceTree validates clusters is an object', () => {
  assert.ok(
    /typeof source\.clusters\s*===\s*'object'/.test(source),
    'should check clusters type'
  );
  assert.ok(
...
test('SentenceTree has componentDidMount', () => {
  assert.ok(
    /componentDidMount\(\)/.test(source),
    'should define componentDidMount'
  );
});
...
test('componentDidMount checks if window data changed before setState', () => {
  ...
  assert.ok(
    ...
    'should use JSON.stringify for deep comparison'
  );
});
...
test('SentenceTree renders empty state when no clusters', () => {
  ...
  assert.ok(
    /No sentences found/.test(source),
    'should show "No sentences found" text'
  );
});

test('SentenceTree empty state shows tag and words', () => {
  assert.ok(
    /Sentence Tree:\s*\{tag\}/.test(source),
    'should display tag in empty state title'
  );
  assert.ok(
    /Words:.*words\.join/.test(source),
    'should display joined words list'
  );
  assert.ok(
    /className="group_title"/.test(source),
    'should have group_title class'
  );
  assert.ok(
    /className="page"/.test(source),
    'should have page class'
  );
});
...
test('Cluster has toggleClusterContent method', () => {
  assert.ok(
    /toggleClusterContent/.test(source),
    'Cluster should define toggleClusterContent'
  );
  assert.ok(
...
test('Cluster renders a toggle button with correct text', () => {
  assert.ok(
    /className="toggle-cluster-btn"/.test(source),
    'should have toggle-cluster-btn class'
  );
  assert.ok(
  ...
  assert.ok(
    /'Collapse'/.test(source) || />Collapse</.test(source),
    'should show Collapse text'
  );
  assert.ok(
    /'Expand'/.test(source) || />Expand</.test(source),
    'should show Expand text'
  );
});
...
test('Cluster hides table when collapsed', () => {
  ...
  assert.ok(
    /className="table-responsive"/.test(source),
    'should have table-responsive container'
  );
});

test('Cluster shows post count from cluster_link', () => {
  assert.ok(
    /posts_count/.test(source),
    'should compute posts_count'
  );
  assert.ok(
...
test('Cluster renders link when cluster_link exists', () => {
  assert.ok(
    /cluster_link\s*\?/.test(source),
    'should conditionally render link'
  );
  assert.ok(
    /href={cluster_link}/.test(source),
    'should use cluster_link as href'
  );
});

test('Cluster renders Cluster label text', () => {
  assert.ok(
    /Cluster\s*\$\{label\}/.test(source),
    'should show Cluster label when no link'
  );
});
...
test('SentenceRow has leftExpanded and rightExpanded state', () => {
  ...
  assert.ok(
    /rightExpanded:\s*false/.test(source),
    'should have rightExpanded state'
  );
});

test('SentenceRow has toggleLeft and toggleRight methods', () => {
  assert.ok(
    /toggleLeft/.test(source),
    'should define toggleLeft'
  );
  assert.ok(
    /toggleRight/.test(source),
    'should define toggleRight'
  );
});

test('SentenceRow renders three cells (left, mid, right)', () => {
  assert.ok(
    /className="left sentence-cell"/.test(source),
    'should have left cell'
  );
  assert.ok(
    /className="mid sentence-cell"/.test(source),
    'should have mid cell'
  );
  assert.ok(
    /className="right sentence-cell"/.test(source),
...
test('SentenceRow left cell renders truncated text', () => {
  assert.ok(
    /renderLeftTruncated/.test(source),
    'should define renderLeftTruncated'
  );
  assert.ok(
...
test('SentenceRow right cell renders truncated text', () => {
  assert.ok(
    /renderTruncated/.test(source),
    'should define renderTruncated'
  );
  assert.ok(
...
test('SentenceRow uses MAX_LEN constant for truncation', () => {
  assert.ok(
    /const MAX_LEN\s*=\s*150/.test(source),
    'should define MAX_LEN as 150'
  );
  assert.ok(
    /length\s*>\s*MAX_LEN/.test(source),
    'should compare text length against MAX_LEN'
  );
});

test('SentenceRow mid cell renders link when post_url exists', () => {
  assert.ok(
    /context\.post_url\s*\?/.test(source),
    'should conditionally render link in mid cell'
  );
  assert.ok(
    /href={context\.post_url}/.test(source),
    'should use context.post_url as href'
  );
  assert.ok(
...
test('SentenceRow mid cell falls back to plain text when no post_url', () => {
  assert.ok(
    /context\.post_url\s*\?[\s\S]*:[\s\S]*context\.mid/.test(source) ||
    /context\.mid/.test(source),
...
test('SentenceRow has show-more-less blocks for truncatable cells', () => {
  ...
  assert.ok(
    /className="show-more-btn"/.test(source),
    'should have show-more-btn class'
  );
  assert.ok(
    /'show more'/.test(source),
    'should show "show more" text'
  );
  assert.ok(
    /'show less'/.test(source),
...
test('SentenceTree render maps clusters to Cluster components', () => {
  assert.ok(
    /Object\.entries\(clusters\)\.map/.test(source),
    'should iterate over cluster entries'
  );
  assert.ok(
    /<Cluster/.test(source),
    'should render Cluster component'
  );
  assert.ok(
...
test('SentenceTree handles null clusters in render check', () => {
  assert.ok(
    /!clusters/.test(source),
    'should handle null clusters'
  );
});
...
test('Cluster label shows posts count in parentheses', () => {
  assert.ok(
    /\(\s*\{posts_count\}/.test(source),
    'should show posts_count in parentheses'
  );
  assert.ok(
    /\{ctxs\.length\}/.test(source),
    'should show ctxs.length'
  );
});

test('Cluster defaults posts_count to at least 1 when link exists', () => {
  assert.ok(
    /posts_count\s*=\s*1/.test(source),
    'should default posts_count to 1'
  );
  assert.ok(
