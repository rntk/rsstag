...
const source = fs.readFileSync(
  new URL('../components/ClustersTopics.js', import.meta.url),
  'utf8'
);
...
test('ClustersTopics loads clusters from window.cluster_data', () => {
  assert.ok(
    /window\.cluster_data/.test(source),
    'component should reference window.cluster_data'
  );
  assert.ok(
...
test('ClustersTopics fetchSentences calls correct API endpoint', () => {
  ...
  assert.ok(
    /URLS\.fetch_sentences/.test(source),
    'should use URLS.fetch_sentences'
  );
  assert.ok(
    /method:\s*['"]POST['"]/.test(source),
    'fetch should use POST method'
  );
});
...
test('ClustersTopics handles cluster with no ranges', () => {
  ...
  assert.ok(
    /ranges:\s*\[\]/.test(source),
    'should set empty ranges when no data'
  );
});
...
test('ClustersTopics uses AbortController for fetch cancellation', () => {
  assert.ok(
    /abortControllerRef/.test(source),
...
test('ClustersTopics defines changeSnippetsStatus for toggling read state', () => {
  assert.ok(
    /changeSnippetsStatus/.test(source),
    'should define changeSnippetsStatus'
  );
  assert.ok(
  ...
  assert.ok(
    /readed:\s*readed/.test(source),
    'should send readed flag in payload'
  );
});

test('ClustersTopics handleToggleRead builds correct selection payload', () => {
  assert.ok(
    /handleToggleRead/.test(source),
    'should define handleToggleRead'
  );
  assert.ok(
    /postId:\s*range\.post_id/.test(source),
    'should include postId in selection'
  );
  assert.ok(
  ...
  assert.ok(
    /rangeKey:\s*rangeKey/.test(source),
    'should include rangeKey in selection'
  );
});

test('ClustersTopics handleReadAll sends all selections', () => {
  assert.ok(
    /handleReadAll/.test(source),
    'should define handleReadAll'
  );
  assert.ok(
    /state\.ranges\.map/.test(source),
    'should map all ranges to selections'
  );
});
...
test('buildRangeKey generates keys from postId and sentenceIndices', () => {
  ...
  assert.ok(
    /String\(postId\)/.test(source),
    'should convert postId to string'
  );
  assert.ok(
  ...
  assert.ok(
    /base\s*\+\s*['"]_['"]/.test(source),
    'should concatenate base with underscore'
  );
});
...
test('ClustersTopics shows "No clusters" when empty', () => {
  assert.ok(
    /clusterIds\.length\s*===\s*0/.test(source),
    'should check if clusterIds is empty'
  );
  assert.ok(
    />No clusters</.test(source),
    'should show "No clusters" text'
  );
});
...
test('ClustersTopics renders cluster list with buttons', () => {
  ...
  assert.ok(
    /className={`clusters-topic-item/.test(source) ||
    /className="clusters-topic-item/.test(source),
    'should render clusters-topic-item buttons'
  );
  assert.ok(
    /is-active/.test(source),
    'should support is-active class for selected cluster'
  );
});

test('ClustersTopics displays cluster title and count', () => {
  assert.ok(
    /className="clusters-topic-title"/.test(source),
    'should have title span'
  );
  assert.ok(
    /className="clusters-topic-count"/.test(source),
  ...
  assert.ok(
    /clusters\[id\]\.count/.test(source),
    'should render cluster count'
  );
});
...
test('ClustersTopics renders sentences panel', () => {
  ...
  assert.ok(
    /className={`clusters-topic-sentences.*isLoading/.test(source) ||
    /is-loading/.test(source),
...
test('ClustersTopics shows placeholder when no cluster selected', () => {
  assert.ok(
    /!currentClusterId/.test(source),
    'should check if no cluster is selected'
  );
  assert.ok(
    /className="clusters-topic-placeholder"/.test(source),
...
test('ClustersTopics renders cluster header for selected cluster', () => {
  assert.ok(
    /className="clusters-topic-header"/.test(source),
    'should have header div'
  );
  assert.ok(
    /currentCluster.*title/.test(source),
    'should show current cluster title in header'
  );
});
...
test('ClustersTopics sentence rows have read/unread classes', () => {
  assert.ok(
    /className={`clusters-topic-sentence.*isRead/.test(source) ||
    /is-read/.test(source),
...
test('ClustersTopics sentence row shows toggle read button', () => {
  ...
  assert.ok(
    /'Mark Read'/.test(source),
    'should show "Mark Read" text'
  );
  assert.ok(
    /'Mark Unread'/.test(source),
    'should show "Mark Unread" text'
  );
  assert.ok(
...
test('ClustersTopics sentence row contains post link', () => {
  assert.ok(
    /className="clusters-topic-sentence-link"/.test(source),
    'should have link class'
  );
  assert.ok(
...
test('ClustersTopics sentence shows meta with topic_title', () => {
  assert.ok(
    /className="clusters-topic-sentence-meta"/.test(source),
    'should have meta div'
  );
  assert.ok(
    /range\.topic_title/.test(source),
    'should show topic_title when available'
  );
});

test('ClustersTopics sentence shows post_id in meta', () => {
  assert.ok(
    /range\.post_id/.test(source),
    'should display post_id in meta'
  );
});
...
test('ClustersTopics toolbar has Read all and Unread all buttons', () => {
  assert.ok(
    /className="clusters-topic-toolbar"/.test(source),
    'should have toolbar container'
  );
  assert.ok(
    /Read all/.test(source),
    'should have Read all button'
  );
  assert.ok(
    /Unread all/.test(source),
    'should have Unread all button'
  );
  assert.ok(
    /className="clusters-read-btn"/.test(source),
    'should use clusters-read-btn class'
  );
});
...
test('ClustersTopics handles fetch errors with .catch', () => {
  assert.ok(
    /\.catch\(\(err\)\s*=>/.test(source),
    'should have catch handler for fetch errors'
  );
  assert.ok(
    /console\.error.*Failed to fetch sentences/.test(source),
    'should log fetch errors'
  );
  assert.ok(
    /AbortError/.test(source),
    'should handle AbortError separately'
  );
});
