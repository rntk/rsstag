...
test('source exports a default class TagToolWidget extending React.Component', () => {
  ...
  assert.ok(
    /export default class TagToolWidget extends React\.Component/.test(src),
...
test('constructor initializes state with hidden true, loading false, tags null, error null', () => {
  const src = readSource();
  assert.ok(
    /this\.state\s*=\s*\{\s*hidden:\s*true,\s*loading:\s*false,\s*tags:\s*null,\s*error:\s*null\s*\}/.test(
      src
    ),
...
test('source imports TagsList', () => {
  const src = readSource();
  assert.ok(/import TagsList from ['"]\.\/tags-list\.js['"]/.test(src), 'should import TagsList');
});

test('source imports rsstag_utils', () => {
  const src = readSource();
  assert.ok(
    /import rsstag_utils from ['"]..\/libs\/rsstag_utils\.js['"]/.test(src),
...
test('loadData hides and clears tags when already shown', () => {
  const src = readSource();
  assert.ok(
    /if\s*\(\s*!this\.state\.hidden\s*\)\s*\{[\s\S]*?this\.setState\(\{\s*hidden:\s*true,\s*tags:\s*null\s*\}\)/.test(
      src
    ),
...
test('loadData clears renderData container on hide', () => {
  const src = readSource();
  assert.ok(/clearRenderData/.test(src), 'should call clearRenderData');
  assert.ok(/container\.innerHTML\s*=\s*['"]{2}/.test(src), 'should clear container innerHTML');
});
...
test('loadData fetches url with encodeURIComponent(tag) via rsstag_utils.fetchJSON', () => {
  const src = readSource();
  assert.ok(
    /rsstag_utils\s*\.\s*fetchJSON\s*\(\s*this\.props\.url\s*\+\s*['"]\/['"]\s*\+\s*encodeURIComponent\(this\.props\.tag\)/.test(
      src
    ),
...
test('on missing data.data, an inline error is set', () => {
  const src = readSource();
  assert.ok(/error:\s*['"]Error\. Try later['"]/.test(src), 'should set the inline error message');
});

test('on fetch rejection, error is logged and inline error is set', () => {
  const src = readSource();
  assert.ok(
    /\.catch\s*\(\s*\(err\)\s*=>\s*\{[\s\S]*?console\.log\(err\)[\s\S]*?error:\s*['"]Error\. Try later['"]/.test(
      src
    ),
...
test('normalizedTags sets root:true on each tag and keys the Map by tag.tag', () => {
  const src = readSource();
  assert.ok(/tag\.root\s*=\s*true/.test(src), 'should set root: true on each tag');
  assert.ok(/tags\.set\(tag\.tag,\s*tag\)/.test(src), 'should key the Map by tag.tag');
});
...
test('render portals TagsList into the list container when not hidden and renderData is absent', () => {
  const src = readSource();
  assert.ok(/createPortal\(\s*<TagsList/.test(src), 'should createPortal a TagsList');
  assert.ok(
    /!this\.state\.hidden\s*&&\s*container/.test(src),
...
test('render resolves the portal container via document.getElementById(props.listContainerId), skipped when renderData is set', () => {
  const src = readSource();
  assert.ok(
    /!this\.props\.renderData\s*&&\s*document\.getElementById\(this\.props\.listContainerId\)/.test(
      src
    ),
...
test('loadData calls renderData(container, tags) imperatively on success when provided', () => {
  const src = readSource();
  assert.ok(
    /if\s*\(\s*this\.props\.renderData\s*\)\s*\{[\s\S]*?this\.props\.renderData\(container,\s*tags\)/.test(
      src
    ),
...
test('componentWillUnmount clears the renderData container', () => {
  const src = readSource();
  assert.ok(
    /componentWillUnmount\s*\(\s*\)\s*\{[\s\S]*?clearRenderData\(\)/.test(src),
