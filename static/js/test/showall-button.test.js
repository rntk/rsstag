...
test('class name is ShowAllButton', () => {
  const src = readSource();
  assert.ok(/export default class ShowAllButton/.test(src),
    'should define class ShowAllButton');
});
...
test('constructor initializes state with ids array and showed boolean', () => {
  const src = readSource();
  assert.ok(/this\.state\s*=\s*\{[\s\S]*?ids\s*:\s*\[\]/.test(src),
    'should set state.ids to empty array');
  assert.ok(/this\.state\s*=\s*\{[\s\S]*?showed\s*:\s*false/.test(src),
...
test('source declares isShowed method', () => {
  const src = readSource();
  assert.ok(/isShowed\s*\(\s*state\s*\)\s*\{/.test(src),
    'should declare isShowed(state) method');
});
...
test('isShowed checks item[1].showed property', () => {
  const src = readSource();
  assert.ok(/item\[1\]\.showed/.test(src),
    'should check item[1].showed');
});

test('isShowed breaks early when showed is true', () => {
  const src = readSource();
  assert.ok(/if\s*\(\s*showed\s*\)\s*\{\s*break\s*;?\s*\}/.test(src),
    'should break early when showed is found');
});

test('isShowed initializes showed to false', () => {
  const src = readSource();
  assert.ok(/let showed\s*=\s*false/.test(src),
    'should initialize showed = false');
});
...
test('componentDidMount binds POSTS_UPDATED event', () => {
  const src = readSource();
  assert.ok(/this\.props\.ES\.bind\s*\(\s*this\.props\.ES\.POSTS_UPDATED\s*,\s*this\.updatePosts/.test(src),
...
test('componentWillUnmount unbinds POSTS_UPDATED event', () => {
  const src = readSource();
  assert.ok(/this\.props\.ES\.unbind\s*\(\s*this\.props\.ES\.POSTS_UPDATED\s*,\s*this\.updatePosts/.test(src),
...
test('source declares changePostsContentState method', () => {
  const src = readSource();
  assert.ok(/changePostsContentState\s*\(\s*e\s*\)\s*\{/.test(src),
    'should declare changePostsContentState(e)');
});

test('changePostsContentState creates data with ids copy and toggled showed', () => {
  const src = readSource();
  assert.ok(/ids\s*:\s*this\.state\.ids\.slice\s*\(\s*0\s*\)/.test(src),
    'should copy ids with slice(0)');
  assert.ok(/showed\s*:\s*!\s*this\.state\.showed/.test(src),
    'should toggle showed state');
});

test('changePostsContentState triggers CHANGE_POSTS_CONTENT_STATE event', () => {
  const src = readSource();
  assert.ok(/this\.props\.ES\.trigger\s*\(\s*this\.props\.ES\.CHANGE_POSTS_CONTENT_STATE/.test(src),
...
test('render binds onClick to changePostsContentState', () => {
  const src = readSource();
  assert.ok(/onClick\s*=\s*\{?\s*this\.changePostsContentState/.test(src),
    'should bind onClick to changePostsContentState');
});

test('render conditionally shows hide/show text', () => {
  const src = readSource();
  assert.ok(/this\.state\.showed\s*\?\s*['"]hide['"]/.test(src),
    'should show "hide" when showed is true');
  assert.ok(/:\s*['"]show['"]/.test(src),
    'should show "show" when showed is false');
});

test('render includes " all" text', () => {
  const src = readSource();
  assert.ok(/['"]?\s*all['"]?\s*</.test(src) || /} all</.test(src),
