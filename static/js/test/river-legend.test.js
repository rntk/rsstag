...
test('source exports a default class', () => {
  ...
  assert.ok(
    /export default class RiverLegend/.test(src),
...
test('constructor accepts container and options parameters', () => {
  const src = readSource();
  assert.ok(
    /constructor\s*\(\s*container\s*,\s*options\s*=\s*\{\}\s*\)/.test(src),
...
test('constructor handles string container with document.querySelector', () => {
  const src = readSource();
  assert.ok(
    /typeof container === ['"]string['"]/.test(src),
...
test('constructor handles DOM element container', () => {
  const src = readSource();
  assert.ok(/:\s*container/.test(src), 'should use container directly if not a string');
});

test('constructor sets items from options with default empty array', () => {
  const src = readSource();
  assert.ok(
    /this\.items\s*=\s*options\.items\s*\|\|\s*\[\]/.test(src),
...
test('constructor sets colorScale from options', () => {
  const src = readSource();
  assert.ok(
    /this\.colorScale\s*=\s*options\.colorScale/.test(src),
...
test('constructor sets onActivate callback with default noop', () => {
  const src = readSource();
  assert.ok(
    /this\.onActivate\s*=\s*options\.onActivate\s*\|\|\s*\(?\s*\(\s*\)\s*=>\s*\{\}\s*\)?/.test(src),
...
test('constructor initializes activeItem to null', () => {
  const src = readSource();
  assert.ok(/this\.activeItem\s*=\s*null/.test(src), 'should initialize activeItem to null');
});

test('constructor sets variant from options with default value', () => {
  const src = readSource();
  assert.ok(
    /this\.variant\s*=\s*options\.variant\s*\|\|\s*['"]default['"]/.test(src),
...
test('source declares update method', () => {
  const src = readSource();
  assert.ok(
    /update\s*\(\s*activeItem\s*\)\s*\{/.test(src),
...
test('update sets this.activeItem', () => {
  const src = readSource();
  assert.ok(/this\.activeItem\s*=\s*activeItem/.test(src), 'should set this.activeItem');
});

test('update calls this.render()', () => {
  const src = readSource();
  assert.ok(/this\.render\s*\(\s*\)/.test(src), 'should call this.render()');
});
...
test('render returns early if container is falsy', () => {
  const src = readSource();
  assert.ok(/!this\.container/.test(src), 'should check for falsy container');
});

test('render returns early if items is falsy', () => {
  const src = readSource();
  assert.ok(/!this\.items/.test(src), 'should check for falsy items');
});

test('render returns early if items array is empty', () => {
  const src = readSource();
  assert.ok(/this\.items\.length\s*===\s*0/.test(src), 'should check for empty items array');
});

test('render clears container innerHTML', () => {
  const src = readSource();
  assert.ok(/this\.container\.innerHTML\s*=\s*['']/.test(src), 'should clear container innerHTML');
});

test('render sets CSS classes with variant', () => {
  const src = readSource();
  assert.ok(
    /this\.container\.className\s*=\s*`river-legend river-legend-\$\{this\.variant\}`/.test(src),
...
test('render applies flex layout styles', () => {
  const src = readSource();
  assert.ok(/display\s*:\s*['"]flex['"]/.test(src), 'should set display to flex');
  assert.ok(/flexWrap\s*:\s*['"]wrap['"]/.test(src), 'should set flexWrap to wrap');
});

test('render uses Object.assign for container styles', () => {
  const src = readSource();
  assert.ok(
    /Object\.assign\s*\(\s*this\.container\.style/.test(src),
...
test('render iterates over items with forEach', () => {
  const src = readSource();
  assert.ok(/this\.items\.forEach/.test(src), 'should iterate over items with forEach');
});

test('render resolves item name from item.name or item itself', () => {
  const src = readSource();
  assert.ok(
    /const name\s*=\s*item\.name\s*\|\|\s*item/.test(src),
...
test('render calculates isActive based on activeItem match', () => {
  const src = readSource();
  assert.ok(
    /const isActive\s*=\s*this\.activeItem\s*===\s*name/.test(src),
...
test('render calculates isDimmed for non-active items', () => {
  const src = readSource();
  assert.ok(
    /const isDimmed\s*=\s*this\.activeItem\s*&&\s*this\.activeItem\s*!==\s*name/.test(src),
...
test('render sets river-legend-item class on item', () => {
  const src = readSource();
  assert.ok(
    /el\.className\s*=\s*['"]river-legend-item['"]/.test(src),
...
test('render handles pill variant padding', () => {
  const src = readSource();
  assert.ok(/this\.variant\s*===\s*['"]pill['"]/.test(src), 'should check for pill variant');
  assert.ok(/['"]6px 12px['"]/.test(src), 'should use 6px 12px padding for pill');
  assert.ok(/['"]4px 8px['"]/.test(src), 'should use 4px 8px padding for default');
});

test('render handles pill variant border and borderRadius', () => {
  const src = readSource();
  assert.ok(/['"]1px solid #ddd['"]/.test(src), 'should set border for pill variant');
  assert.ok(/['"]20px['"]/.test(src), 'should set borderRadius 20px for pill');
  assert.ok(/['"]4px['"]/.test(src), 'should set borderRadius 4px for default');
});

test('render handles pill variant colorBox shape', () => {
  const src = readSource();
  assert.ok(
    /borderRadius\s*:\s*this\.variant\s*===\s*['"]pill['"]\s*\?\s*['"]50%['"]/.test(src),
    'should set circular colorBox for pill'
  );
  assert.ok(/['"]2px['"]/.test(src), 'should set square colorBox for default');
});
...
test('render creates colorBox element', () => {
  const src = readSource();
  assert.ok(
    /const colorBox\s*=\s*document\.createElement\s*\(\s*['"]div['"]\s*\)/.test(src),
...
test('render calls colorScale(name) for background color', () => {
  const src = readSource();
  assert.ok(
    /this\.colorScale\s*\(\s*name\s*\)/.test(src),
...
test('colorBox has 12px dimensions', () => {
  const src = readSource();
  assert.ok(/width\s*:\s*['"]12px['"]/.test(src), 'should set 12px width');
  assert.ok(/height\s*:\s*['"]12px['"]/.test(src), 'should set 12px height');
});

test('render creates label span element', () => {
  const src = readSource();
  assert.ok(
    /const label\s*=\s*document\.createElement\s*\(\s*['"]span['"]\s*\)/.test(src),
...
test('render sets label textContent to name', () => {
  const src = readSource();
  assert.ok(/label\.textContent\s*=\s*name/.test(src), 'should set label textContent');
});

test('render appends colorBox and label to item', () => {
  const src = readSource();
  assert.ok(/el\.appendChild\s*\(\s*colorBox\s*\)/.test(src), 'should append colorBox');
  assert.ok(/el\.appendChild\s*\(\s*label\s*\)/.test(src), 'should append label');
});

test('render appends item to container', () => {
  const src = readSource();
  assert.ok(
    /this\.container\.appendChild\s*\(\s*el\s*\)/.test(src),
...
test('render adds mouseenter event listener', () => {
  const src = readSource();
  assert.ok(
    /addEventListener\s*\(\s*['"]mouseenter['"]/.test(src),
    'should add mouseenter listener'
  );
  assert.ok(
    /this\.onActivate\s*\(\s*name\s*\)/.test(src),
...
test('render adds mouseleave event listener', () => {
  const src = readSource();
  assert.ok(
    /addEventListener\s*\(\s*['"]mouseleave['"]/.test(src),
    'should add mouseleave listener'
  );
  assert.ok(
    /this\.onActivate\s*\(\s*null\s*\)/.test(src),
...
test('source has no React import', () => {
  const src = readSource();
  assert.ok(!/import.*React/.test(src), 'should not import React');
});
