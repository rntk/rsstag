...
test('class name is PostsNumbers', () => {
  const src = readSource();
  assert.ok(/export default class PostsNumbers/.test(src), 'should define class PostsNumbers');
});
...
test('constructor initializes _default_state with read, unread, all', () => {
  const src = readSource();
  assert.ok(
    /this\._default_state\s*=\s*\{[\s\S]*?read\s*:\s*0/.test(src),
    'should set _default_state.read to 0'
  );
  assert.ok(
    /this\._default_state\s*=\s*\{[\s\S]*?unread\s*:\s*0/.test(src),
    'should set _default_state.unread to 0'
  );
  assert.ok(
    /this\._default_state\s*=\s*\{[\s\S]*?all\s*:\s*0/.test(src),
...
test('constructor copies _default_state into state', () => {
  const src = readSource();
  assert.ok(
    /Object\.assign\s*\(\s*\{\s*\}\s*,\s*this\._default_state\s*\)/.test(src),
...
test('constructor binds updateNumbers method', () => {
  const src = readSource();
  assert.ok(
    /this\.updateNumbers\s*=\s*this\.updateNumbers\.bind\(this\)/.test(src),
...
test('source declares updateNumbers method', () => {
  const src = readSource();
  assert.ok(
    /updateNumbers\s*\(\s*posts_state\s*\)\s*\{/.test(src),
...
test('updateNumbers iterates over posts_state.posts', () => {
  const src = readSource();
  assert.ok(
    /for\s*\(\s*let item of posts_state\.posts/.test(src),
...
test('updateNumbers accesses post via item[1]', () => {
  const src = readSource();
  assert.ok(/let post\s*=\s*item\[1\]/.test(src), 'should get post from item[1]');
});

test('updateNumbers checks post.post.read for read/unread', () => {
  const src = readSource();
  assert.ok(/if\s*\(\s*post\.post\.read\s*\)/.test(src), 'should check post.post.read');
});

test('updateNumbers increments state.read for read posts', () => {
  const src = readSource();
  assert.ok(/state\.read\+\+/.test(src), 'should increment state.read');
});

test('updateNumbers increments state.unread for unread posts', () => {
  const src = readSource();
  assert.ok(/state\.unread\+\+/.test(src), 'should increment state.unread');
});

test('updateNumbers calculates state.all as sum of read and unread', () => {
  const src = readSource();
  assert.ok(
    /state\.all\s*=\s*state\.unread\s*\+\s*state\.read/.test(src),
...
test('componentDidMount binds POSTS_UPDATED event', () => {
  const src = readSource();
  assert.ok(
    /this\.props\.ES\.bind\s*\(\s*this\.props\.ES\.POSTS_UPDATED\s*,\s*this\.updateNumbers/.test(
      src
    ),
...
test('componentWillUnmount unbinds POSTS_UPDATED event', () => {
  const src = readSource();
  assert.ok(
    /this\.props\.ES\.unbind\s*\(\s*this\.props\.ES\.POSTS_UPDATED\s*,\s*this\.updateNumbers/.test(
      src
    ),
...
test('render checks this.state for truthy render path', () => {
  const src = readSource();
  assert.ok(/if\s*\(\s*this\.state\s*\)/.test(src), 'should check if(this.state)');
});

test('render returns p with unread / read format', () => {
  const src = readSource();
  assert.ok(/<p>/.test(src), 'should render a p element');
  assert.ok(/\{this\.state\.unread\}/.test(src), 'should render state.unread');
  assert.ok(/\/ /.test(src), 'should include " / " separator');
  assert.ok(/\{this\.state\.read\}/.test(src), 'should render state.read');
});

test('render has fallback path showing 0/0', () => {
  const src = readSource();
  assert.ok(/<p>0\/0<\/p>/.test(src), 'should render "0/0" fallback');
});
