...
test('class name is PostsWordTree', () => {
  const src = readSource();
  assert.ok(/export default class PostsWordTree/.test(src),
    'should define class PostsWordTree');
});
...
test('constructor initializes _renderred flag to false', () => {
  const src = readSource();
  assert.ok(/this\._renderred\s*=\s*false/.test(src),
    'should set this._renderred = false');
});
...
test('updateWordTree skips category group', () => {
  const src = readSource();
  assert.ok(/data\.group\s*===\s*['"]category['"]/.test(src),
    'should check for group === "category"');
  assert.ok(/return/.test(src),
    'should return early for category');
});

test('updateWordTree skips feed group', () => {
  const src = readSource();
  assert.ok(/data\.group\s*===\s*['"]feed['"]/.test(src),
    'should check for group === "feed"');
});

test('updateWordTree skips if already rendered', () => {
  const src = readSource();
  assert.ok(/if\s*\(\s*this\._renderred\s*\)/.test(src),
    'should check this._renderred');
  assert.ok(/return/.test(src),
    'should return if already rendered');
});
...
test('updateWordTree sets _renderred flag after validation', () => {
  const src = readSource();
  assert.ok(/this\._renderred\s*=\s*true/.test(src),
    'should set this._renderred = true');
});
...
test('updateWordTree uses setOnLoadCallback', () => {
  const src = readSource();
  assert.ok(/google\.charts\.setOnLoadCallback/.test(src),
    'should use setOnLoadCallback');
});
...
test('updateWordTree splits group_title by space to get tags', () => {
  const src = readSource();
  assert.ok(/data\.group_title\.split\(\s*['"] ['"]\s*\)/.test(src),
    'should split group_title by space');
  assert.ok(/let tags\s*=/.test(src),
    'should store tags in tags variable');
});

test('updateWordTree uses window size of 10', () => {
  const src = readSource();
  assert.ok(/let window\s*=\s*10/.test(src),
    'should set window = 10');
});

test('updateWordTree iterates over tags', () => {
  const src = readSource();
  assert.ok(/for\s*\(\s*let i\s*=\s*0/.test(src),
    'should iterate over tags');
  assert.ok(/i\s*<\s*tags\.length/.test(src),
    'should iterate by tag index');
});
...
test('updateWordTree creates a div container per tag', () => {
  ...
  assert.ok(/container\.id\s*=\s*['"]wordtree['"]/.test(src),
