...
test('class name is PostsItem', () => {
  const src = readSource();
  assert.ok(/export default class PostsItem/.test(src), 'should define class PostsItem');
});
...
test('constructor initializes state with post, tag, words', () => {
  const src = readSource();
  assert.ok(
    /this\.state\s*=\s*\{[\s\S]*?post\s*:\s*props\.post/.test(src),
    'should set state.post from props.post'
  );
  assert.ok(
    /this\.state\s*=\s*\{[\s\S]*?tag\s*:\s*props\.tag/.test(src),
    'should set state.tag from props.tag'
  );
  assert.ok(
    /this\.state\s*=\s*\{[\s\S]*?words\s*:\s*props\.words/.test(src),
...
test('constructor initializes showed as false', () => {
  const src = readSource();
  assert.ok(/this\.showed\s*=\s*false/.test(src), 'should initialize this.showed = false');
});

test('constructor binds clickReadButton method', () => {
  const src = readSource();
  assert.ok(
    /this\.clickReadButton\s*=\s*this\.clickReadButton\.bind\(this\)/.test(src),
...
test('constructor binds showPostLinks method', () => {
  const src = readSource();
  assert.ok(
    /this\.showPostLinks\s*=\s*this\.showPostLinks\.bind\(this\)/.test(src),
...
test('constructor binds setCurrent method', () => {
  const src = readSource();
  assert.ok(
    /this\.setCurrent\s*=\s*this\.setCurrent\.bind\(this\)/.test(src),
...
test('constructor binds getNode method', () => {
  const src = readSource();
  assert.ok(/this\.getNode\s*=\s*this\.getNode\.bind\(this\)/.test(src), 'should bind getNode');
});

test('constructor initializes stopw from stopwords()', () => {
  const src = readSource();
  assert.ok(
    /this\.stopw\s*=\s*stopwords\s*\(\s*\)/.test(src),
...
test('source declares setCurrent method', () => {
  const src = readSource();
  assert.ok(/setCurrent\s*\(\s*\)\s*\{/.test(src), 'should declare setCurrent() method');
});

test('setCurrent triggers SET_CURRENT_POST event', () => {
  const src = readSource();
  assert.ok(/SET_CURRENT_POST/.test(src), 'should reference SET_CURRENT_POST event');
  assert.ok(
    /this\.props\.ES\.trigger\s*\(\s*this\.props\.ES\.SET_CURRENT_POST/.test(src),
...
test('source declares clickReadButton method', () => {
  const src = readSource();
  assert.ok(/clickReadButton\s*\(\s*\)\s*\{/.test(src), 'should declare clickReadButton() method');
});

test('clickReadButton triggers CHANGE_POSTS_STATUS event', () => {
  const src = readSource();
  assert.ok(/CHANGE_POSTS_STATUS/.test(src), 'should reference CHANGE_POSTS_STATUS event');
  assert.ok(/readed\s*:/.test(src), 'should pass readed in payload');
});

test('source declares showPostLinks method', () => {
  const src = readSource();
  assert.ok(/showPostLinks\s*\(\s*\)\s*\{/.test(src), 'should declare showPostLinks() method');
});

test('showPostLinks triggers SHOW_POST_LINKS event', () => {
  const src = readSource();
  assert.ok(/SHOW_POST_LINKS/.test(src), 'should reference SHOW_POST_LINKS event');
});

test('source declares changePostsContentState method', () => {
  const src = readSource();
  assert.ok(
    /changePostsContentState\s*\(\s*\)\s*\{/.test(src),
...
test('changePostsContentState triggers CHANGE_POSTS_CONTENT_STATE event', () => {
  const src = readSource();
  assert.ok(
    /CHANGE_POSTS_CONTENT_STATE/.test(src),
...
test('source declares getNode method', () => {
  const src = readSource();
  assert.ok(/getNode\s*\(/.test(src), 'should declare getNode() method');
});

test('getNode stores node reference as this.node', () => {
  const src = readSource();
  assert.ok(/this\.node\s*=\s*node/.test(src), 'should store node as this.node');
});
...
test('source declares componentDidUpdate lifecycle method', () => {
  const src = readSource();
  assert.ok(
    /componentDidUpdate\s*\(\s*\)\s*\{/.test(src),
...
test('componentDidUpdate checks post.current and showed state', () => {
  const src = readSource();
  assert.ok(/this\.state\.post\.current/.test(src), 'should check state.post.current');
});

test('componentDidUpdate calls window.scrollTo', () => {
  const src = readSource();
  assert.ok(/window\.scrollTo/.test(src), 'should call window.scrollTo');
});

test('componentDidUpdate uses node.offsetTop', () => {
  const src = readSource();
  assert.ok(
    /this\.node\.offsetTop/.test(src),
...
test('render returns div with post CSS class', () => {
  const src = readSource();
  assert.ok(
    /className.*['"]post['"]/.test(src) || /className.*['"]post /.test(src),
...
test('render uses current_post class conditionally', () => {
  const src = readSource();
  assert.ok(/current_post/.test(src), 'should conditionally apply current_post class');
});

test('render uses ref callback with getNode', () => {
  const src = readSource();
  assert.ok(/ref\s*=\s*\{?\s*this\.getNode/.test(src), 'should use getNode as ref callback');
});

test('render sets onClick to setCurrent on main div', () => {
  const src = readSource();
  assert.ok(/onClick\s*=\s*\{?\s*this\.setCurrent/.test(src), 'should bind onClick to setCurrent');
});

test('render includes anchor with post position name', () => {
  const src = readSource();
  assert.ok(/<a name/.test(src), 'should include anchor with name attribute');
});

test('render includes post_title CSS class', () => {
  const src = readSource();
  assert.ok(/post_title/.test(src), 'should use post_title CSS class');
});

test('render includes post_title_link CSS class', () => {
  const src = readSource();
  assert.ok(/post_title_link/.test(src), 'should use post_title_link CSS class');
});
...
test('render includes post_meta CSS class', () => {
  const src = readSource();
  assert.ok(/post_meta/.test(src), 'should use post_meta CSS class');
});
...
test('render includes post_content CSS class', () => {
  const src = readSource();
  assert.ok(/post_content/.test(src), 'should use post_content CSS class');
});

test('render uses hide class conditionally for content', () => {
  const src = readSource();
  assert.ok(/hide/.test(src), 'should conditionally apply hide class to content');
});

test('render includes post_tag_contexts CSS class', () => {
  const src = readSource();
  assert.ok(/post_tag_contexts/.test(src), 'should use post_tag_contexts CSS class');
});

test('render includes post_tag_context_tag CSS class', () => {
  const src = readSource();
  assert.ok(
    /post_tag_context_tag/.test(src),
...
test('render includes post_tools CSS class', () => {
  const src = readSource();
  assert.ok(/post_tools/.test(src), 'should use post_tools CSS class');
});

test('render includes post_show_content span', () => {
  const src = readSource();
  assert.ok(/post_show_content/.test(src), 'should use post_show_content CSS class');
});

test('render shows Show/Hide post text', () => {
  const src = readSource();
  assert.ok(/Hide/.test(src) && /Show/.test(src), 'should include Show/Hide post text');
});

test('render includes post_show_links span', () => {
  const src = readSource();
  assert.ok(/post_show_links/.test(src), 'should use post_show_links CSS class');
});

test('render includes read_button CSS class', () => {
  const src = readSource();
  assert.ok(/read_button/.test(src), 'should use read_button CSS class');
});

test('render uses read/unread class conditionally', () => {
  const src = readSource();
  assert.ok(
    /['"]read['"].*['"]unread['"]|['"]unread['"].*['"]read['"]/.test(src) ||
...
test('render includes download_button CSS class', () => {
  const src = readSource();
  assert.ok(/download_button/.test(src), 'should use download_button CSS class');
});

test('render includes download link with /download/posts/ path', () => {
  const src = readSource();
  assert.ok(
    /\/download\/posts\/\$\{.*pos/.test(src),
...
test('render includes post_links_content CSS class', () => {
  const src = readSource();
  assert.ok(/post_links_content/.test(src), 'should use post_links_content CSS class');
});
...
test('render includes post_tag_link CSS class', () => {
  const src = readSource();
  assert.ok(/post_tag_link/.test(src), 'should use post_tag_link CSS class for tag links');
});

test('render includes post_tag_letter_block CSS class', () => {
  const src = readSource();
  assert.ok(
    /post_tag_letter_block/.test(src),
...
test('render includes post_tag_letter CSS class', () => {
  const src = readSource();
  assert.ok(/post_tag_letter/.test(src), 'should use post_tag_letter CSS class');
});

test('render includes Cluster link conditionally', () => {
  const src = readSource();
  assert.ok(/Cluster/.test(src), 'should include Cluster link');
  assert.ok(/clst_url/.test(src), 'should check clst_url for cluster link');
});
...
test('render uses No Title fallback for empty titles', () => {
  const src = readSource();
  assert.ok(/No Title/.test(src), 'should use "No Title" for empty post titles');
});

test('render includes post position with # prefix', () => {
  const src = readSource();
  assert.ok(/#\{.*pos/.test(src), 'should display post position with #');
});

test('render displays clusters when present', () => {
  const src = readSource();
  assert.ok(
    /post\.post\.clusters/.test(src) || /\.clusters/.test(src),
...
test('render includes "Show links" text', () => {
  const src = readSource();
  assert.ok(/Show links/.test(src), 'should include Show links text');
});
...
test('source imports stopwords from libs', () => {
  const src = readSource();
  assert.ok(/import.*stopwords.*from.*libs/.test(src), 'should import stopwords from libs');
});
