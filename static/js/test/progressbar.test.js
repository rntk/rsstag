...
test('class name is ProgressBar', () => {
  const src = readSource();
  assert.ok(/export default class ProgressBar/.test(src), 'should define class ProgressBar');
});
...
test('constructor initializes state with tasks array and progress 0', () => {
  const src = readSource();
  assert.ok(
    /this\.state\s*=\s*\{[\s\S]*?tasks\s*:\s*\[\]/.test(src),
    'should set state.tasks to empty array'
  );
  assert.ok(
    /this\.state\s*=\s*\{[\s\S]*?progress\s*:\s*0/.test(src),
...
test('constructor binds changeFilling method', () => {
  const src = readSource();
  assert.ok(
    /this\.changeFilling\s*=\s*this\.changeFilling\.bind\(this\)/.test(src),
...
test('constructor binds animationEnd method', () => {
  const src = readSource();
  assert.ok(
    /this\.animationEnd\s*=\s*this\.animationEnd\.bind\(this\)/.test(src),
...
test('componentDidMount binds CHANGE_PROGRESSBAR event', () => {
  const src = readSource();
  assert.ok(
    /this\.props\.ES\.bind\s*\(\s*this\.props\.ES\.CHANGE_PROGRESSBAR\s*,\s*this\.changeFilling/.test(
      src
    ),
...
test('componentWillUnmount unbinds CHANGE_PROGRESSBAR event', () => {
  const src = readSource();
  assert.ok(
    /this\.props\.ES\.unbind\s*\(\s*this\.props\.ES\.CHANGE_PROGRESSBAR\s*,\s*this\.changeFilling/.test(
      src
    ),
...
test('source declares animationEnd method', () => {
  const src = readSource();
  assert.ok(/animationEnd\s*\(\s*\)\s*\{/.test(src), 'should declare animationEnd() method');
});

test('animationEnd triggers PROGRESSBAR_ANIMATION_END event', () => {
  const src = readSource();
  assert.ok(
    /this\.props\.ES\.trigger\s*\(\s*this\.props\.ES\.PROGRESSBAR_ANIMATION_END/.test(src),
...
test('source declares changeFilling method', () => {
  const src = readSource();
  assert.ok(
    /changeFilling\s*\(\s*state\s*\)\s*\{/.test(src),
...
test('render returns div with filling CSS class', () => {
  const src = readSource();
  assert.ok(/className\s*=\s*['"]filling['"]/.test(src), 'should set className="filling"');
});

test('render uses onTransitionEnd for animationEnd', () => {
  const src = readSource();
  assert.ok(
    /onTransitionEnd\s*=\s*\{?\s*this\.animationEnd/.test(src),
...
test('render sets display based on progress > 0', () => {
  const src = readSource();
  assert.ok(
    /this\.state\.progress\s*>\s*0\s*\?\s*['"]block['"]/.test(src),
    'should display "block" when progress > 0'
  );
  assert.ok(/:\s*['"]none['"]/.test(src), 'should display "none" when progress <= 0');
});

test('render sets width to progress percentage', () => {
  const src = readSource();
  assert.ok(
    /width\s*:\s*this\.state\.progress\s*\+\s*['"]%['"]/.test(src),
...
test('render style object contains display and width', () => {
  const src = readSource();
  assert.ok(/display\s*:/.test(src), 'should have display in style');
  assert.ok(/width\s*:/.test(src), 'should have width in style');
});

test('render div is self-closing (no children)', () => {
  const src = readSource();
  assert.ok(/<div className="filling"[^>]*><\/div>/.test(src), 'should render empty div');
});
