...
test('source declares TopicsRiverChart as a default export class', () => {
  assert.ok(/export\s+default\s+class\s+TopicsRiverChart\b/.test(source), 'should export default class TopicsRiverChart');
});

test('source declares constructor with containerSelector and options params', () => {
  assert.ok(/constructor\s*\(\s*containerSelector\s*,\s*options\s*=\s*\{/.test(source), 'constructor should accept containerSelector and options');
});
...
test('source handles both string selector and element for container', () => {
  assert.ok(/typeof\s+containerSelector\s*===\s*['"]string['"]/.test(source), 'should check if selector is string');
  assert.ok(/document\.querySelector\s*\(\s*containerSelector\s*\)/.test(source), 'should query selector if string');
});

test('source reads topics from options', () => {
  assert.ok(/this\.topics\s*=\s*options\.topics\s*\|\|\s*\[\]/.test(source), 'should set topics from options');
});

test('source reads articleLength from options', () => {
  assert.ok(/this\.articleLength\s*=\s*options\.articleLength\s*\|\|\s*0/.test(source), 'should set articleLength from options');
});
...
test('source calls _calculateEffectiveLength in constructor', () => {
  assert.ok(/this\.effectiveLength\s*=\s*this\._calculateEffectiveLength\(\)/.test(source), 'should call _calculateEffectiveLength');
});

test('source derives keys from topic names', () => {
  assert.ok(/this\.keys\s*=\s*this\.topics\.map\s*\(\s*\(\s*t\s*\)\s*=>\s*t\.name\s*\)/.test(source), 'should map topic names to keys');
});

test('source creates colorScale with getRiverColorScale', () => {
  assert.ok(/this\.colorScale\s*=\s*getRiverColorScale\s*\(\s*this\.keys\s*\)/.test(source), 'should create color scale from keys');
});
...
test('source declares _calculateEffectiveLength method', () => {
  assert.ok(/_calculateEffectiveLength\s*\(\s*\)/.test(source), 'should have _calculateEffectiveLength method');
});
...
test('_calculateEffectiveLength returns 0 for empty topics', () => {
  assert.ok(/if\s*\(!this\.topics\s*\|\|\s*this\.topics\.length\s*===\s*0\)\s*return\s*0/.test(source), 'should return 0 for empty topics');
});

test('_calculateEffectiveLength finds max sentence index', () => {
  assert.ok(/Math\.max\s*\(\s*\.\.\.\s*topic\.sentences/.test(source), 'should use Math.max with spread on sentences');
});
...
test('_calculateEffectiveLength caps at valid articleLength', () => {
  assert.ok(/Math\.min\s*\(\s*maxSentenceIndex\s*\+\s*5\s*,\s*validArticleLength/.test(source), 'should cap at articleLength');
});

test('_calculateEffectiveLength validates articleLength is positive number', () => {
  assert.ok(/typeof\s+this\.articleLength\s*===\s*['"]number['"]\s*&&\s*this\.articleLength\s*>\s*0/.test(source), 'should validate articleLength');
});
...
test('init checks for d3 availability', () => {
  assert.ok(/typeof\s+d3\s*===\s*['"]undefined['"]/.test(source), 'should check if d3 is undefined');
});
...
test('init sets container styles with Object.assign', () => {
  assert.ok(/Object\.assign\s*\(\s*this\.container\.style/.test(source), 'should use Object.assign for styles');
});
...
test('init creates chart wrapper with height 520px', () => {
  assert.ok(/chartWrapper\.style\.height\s*=\s*['"]520px['"]/.test(source), 'chart wrapper should be 520px');
});

test('init creates SVG element using createElementNS', () => {
  assert.ok(/createElementNS\s*\(\s*['"]http:\/\/www\.w3\.org\/2000\/svg['"]/.test(source), 'should create SVG via createElementNS');
});
...
test('init registers window resize listener', () => {
  assert.ok(/window\.addEventListener\s*\(\s*['"]resize['"]/.test(source), 'should register resize listener');
});
...
test('setActiveTopic calls legend.update when legend available', () => {
  assert.ok(/if\s*\(\s*this\.legend\s*\)\s*this\.legend\.update\s*\(\s*name\s*\)/.test(source), 'should update legend');
});
...
test('_updateStyles selects .stream-layer paths', () => {
  assert.ok(/this\.svg\.selectAll\s*\(\s*['"]\.stream-layer['"]/.test(source), 'should select stream-layer paths');
});

test('_updateStyles dims inactive topics to opacity 0.2', () => {
  assert.ok(/opacity['"]\s*,\s*\(\s*d\s*\)\s*=>\s*\(\s*d\.key\s*===\s*this\.activeTopic\s*\?\s*1\s*:\s*0\.2/.test(source), 'should dim inactive to 0.2');
});

test('_updateStyles sets active topic stroke to #333', () => {
  assert.ok(/stroke['"]\s*,\s*\(d\)\s*=>\s*\(\s*d\.key\s*===\s*this\.activeTopic\s*\?\s*['"]#333['"]/.test(source), 'should set active stroke');
});

test('_updateStyles resets all paths when no active topic', () => {
  assert.ok(/\.transition\(\)\.duration\(200\)\.style\('opacity',\s*0\.85\)/.test(source), 'should reset opacity to 0.85');
});
...
test('render returns early when topics is empty', () => {
  assert.ok(/!this\.topics\s*\|\|\s*this\.topics\.length\s*===\s*0/.test(source), 'should check topics');
});

test('render calculates binCount from containerWidth', () => {
  assert.ok(/Math\.max\s*\(\s*15\s*,\s*Math\.min\s*\(\s*60\s*,\s*Math\.floor\s*\(\s*containerWidth\s*\/\s*40/.test(source), 'should calculate binCount between 15 and 60');
});

test('render calls calculateBins', () => {
  assert.ok(/calculateBins\s*\(\s*binCount\s*,\s*this\.topics/.test(source), 'should call calculateBins');
});
...
test('render calls estimateCharacterCounts', () => {
  assert.ok(/estimateCharacterCounts\s*\(\s*data/.test(source), 'should call estimateCharacterCounts');
});
...
test('render creates SVG with d3.select', () => {
  assert.ok(/this\.svg\s*=\s*d3\.select\s*\(\s*svgEl\s*\)/.test(source), 'should use d3.select on SVG element');
});
...
test('render appends .stream-layer paths with fill from colorScale', () => {
  assert.ok(/\.attr\s*\(\s*['"]class['"]\s*,\s*['"]stream-layer['"]/.test(source), 'should set stream-layer class');
  assert.ok(/\.style\s*\(\s*['"]fill['"]\s*,\s*\(\s*d\s*\)\s*=>\s*this\.colorScale\s*\(\s*d\.key\s*\)/.test(source), 'should use colorScale for fill');
});
...
test('render creates tooltip with river-tooltip class', () => {
  ...
  assert.ok(/position['"]\s*,\s*['"]absolute['"]/.test(source), 'tooltip should be absolute positioned');
});
...
test('render creates x-axis with axisBottom', () => {
  ...
  assert.ok(/\.attr\s*\(\s*['"]class['"]\s*,\s*['"]x-axis['"]/.test(source), 'should set x-axis class');
});

test('render creates y-axis with axisLeft', () => {
  assert.ok(/d3\s*\.\s*axisLeft/.test(source), 'should use d3.axisLeft');
  assert.ok(/\.attr\s*\(\s*['"]class['"]\s*,\s*['"]y-axis['"]/.test(source), 'should set y-axis class');
});
...
test('render creates stream labels at thickest point', () => {
  assert.ok(/\.attr\s*\(\s*['"]class['"]\s*,\s*['"]stream-label['"]/.test(source), 'should set stream-label class');
});

test('render limits stream labels to top 8 by thickness', () => {
  assert.ok(/\.slice\s*\(\s*0\s*,\s*Math\.min\s*\(\s*8/.test(source), 'should limit to top 8 labels');
});
