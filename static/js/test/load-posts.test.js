...
test('source exports a named class LoadPosts (not default)', () => {
  ...
  assert.ok(/export class LoadPosts/.test(src),
    'should export a named class LoadPosts');
  assert.ok(!/export default class LoadPosts/.test(src),
    'should not be a default export');
});

test('class extends React.Component', () => {
  const src = readSource();
  assert.ok(/class LoadPosts extends React\.Component/.test(src),
...
test('constructor binds loadMore method', () => {
  const src = readSource();
  assert.ok(/this\.loadMore\s*=\s*this\.loadMore\.bind\(this\)/.test(src),
    'should bind loadMore');
});
...
test('source declares loadMore method', () => {
  const src = readSource();
  assert.ok(/loadMore\s*\(\s*\)\s*\{/.test(src),
    'should declare loadMore() method');
});

test('loadMore triggers LOAD_MORE_POSTS event', () => {
  const src = readSource();
  assert.ok(/this\.props\.ES\.trigger\s*\(\s*this\.props\.ES\.LOAD_MORE_POSTS/.test(src),
    'should trigger LOAD_MORE_POSTS');
});

test('loadMore does not pass any payload', () => {
  const src = readSource();
  assert.ok(/LOAD_MORE_POSTS\s*\)/.test(src),
    'should trigger LOAD_MORE_POSTS without payload');
});
...
test('render returns div with load_more_posts CSS class', () => {
  const src = readSource();
  assert.ok(/className\s*=\s*['"]load_more_posts['"]/.test(src),
...
test('render binds onClick to loadMore', () => {
  const src = readSource();
  assert.ok(/onClick\s*=\s*\{?\s*this\.loadMore/.test(src),
    'should bind onClick to loadMore');
});

test('render displays "Load more" text on button', () => {
  const src = readSource();
  assert.ok(/Load more/.test(src),
    'should display "Load more" on button');
});
