...
test('source exports PostsTags as a named function', () => {
  ...
  assert.ok(/export function PostsTags\s*\(\s*state\s*\)/.test(src),
    'should export function PostsTags(state)');
});

test('PostsTags accepts a single state parameter', () => {
  const src = readSource();
  assert.ok(/PostsTags\s*\(\s*state\s*\)/.test(src),
...
test('PostsTags checks for falsy state', () => {
  const src = readSource();
  assert.ok(/!state/.test(src),
    'should check !state');
});

test('PostsTags checks for state.posts existence', () => {
  const src = readSource();
  assert.ok(/!state\.posts/.test(src),
    'should check !state.posts');
});
...
test('PostsTags creates freq accumulator object', () => {
  const src = readSource();
  assert.ok(/let freq\s*=\s*\{\}/.test(src),
    'should create empty freq object');
});

test('PostsTags creates tags_l array', () => {
  const src = readSource();
  assert.ok(/let tags_l\s*=\s*\[\]/.test(src),
    'should create tags_l array');
});
...
test('PostsTags iterates over post.post.tags', () => {
  const src = readSource();
  assert.ok(/for\s*\(\s*let tag of post\.post\.tags/.test(src),
    'should iterate over post.post.tags');
});

test('PostsTags initializes new tag frequency to 0', () => {
  const src = readSource();
  assert.ok(/freq\[tag\]\s*=\s*0/.test(src),
    'should initialize freq[tag] = 0');
});

test('PostsTags pushes new tag to tags_l array', () => {
  const src = readSource();
  assert.ok(/tags_l\.push\s*\(\s*tag\s*\)/.test(src),
    'should push tag to tags_l');
});

test('PostsTags increments tag frequency counter', () => {
  const src = readSource();
  assert.ok(/freq\[tag\]\+\+/.test(src),
    'should increment freq[tag]');
});
...
test('PostsTags sorts tags_l array', () => {
  const src = readSource();
  assert.ok(/tags_l\.sort\s*\(/.test(src),
    'should call sort on tags_l');
});

test('PostsTags sorts by frequency descending (highest first)', () => {
  const src = readSource();
  assert.ok(/freq\[a\]\s*<\s*freq\[b\]/.test(src),
    'should compare freq[a] < freq[b]');
  assert.ok(/return 1/.test(src),
...
test('PostsTags passes key prop to TagItem', () => {
  const src = readSource();
  assert.ok(/key\s*=\s*\{?\s*tg/.test(src),
    'should pass key=tg');
});

test('PostsTags passes tag prop to TagItem', () => {
  const src = readSource();
  assert.ok(/tag\s*=\s*\{?\s*tag\s*\}/.test(src),
    'should pass tag object');
});

test('PostsTags passes empty tags array to TagItem', () => {
  const src = readSource();
  assert.ok(/tags\s*=\s*\{?\s*\[\s*\]/.test(src),
    'should pass tags=[]');
});

test('PostsTags passes tag_hash prop to TagItem', () => {
  const src = readSource();
  assert.ok(/tag_hash\s*=\s*\{?\s*tg/.test(src),
    'should pass tag_hash=tg');
});

test('PostsTags passes uniq_id prop to TagItem', () => {
  const src = readSource();
  assert.ok(/uniq_id\s*=\s*\{?\s*tg/.test(src),
    'should pass uniq_id=tg');
});

test('PostsTags passes is_bigram prop as false', () => {
  const src = readSource();
  assert.ok(/is_bigram\s*=\s*\{?\s*false/.test(src),
    'should pass is_bigram=false');
});
