...
test('source exports a default class BiGramsMentionsChart', () => {
  ...
  assert.ok(/export default class BiGramsMentionsChart/.test(src),
    'should export default class BiGramsMentionsChart');
});

test('constructor accepts container_id and event_system parameters', () => {
  const src = readSource();
  assert.ok(/constructor\s*\(\s*container_id\s*,\s*event_system/.test(src),
...
test('source declares getDates method', () => {
  const src = readSource();
  assert.ok(/getDates\s*\(\s*data\s*,\s*skip/.test(src),
    'should declare getDates(data, skip) method');
});

test('getDates iterates over data.bigrams', () => {
  const src = readSource();
  assert.ok(/data\.bigrams/.test(src),
    'should access data.bigrams');
  assert.ok(/for\s*\(\s*let\s+bi\s+in\s+data\.bigrams/.test(src),
    'should iterate over bigrams object');
});

test('getDates calculates sums for each bigram', () => {
  const src = readSource();
  assert.ok(/sum\s*\+?=/.test(src),
    'should accumulate sum');
  assert.ok(/Number\.parseInt/.test(src),
    'should use Number.parseInt');
});

test('getDates uses Set for sums tracking', () => {
  const src = readSource();
  assert.ok(/sums\s*=\s*new Set/.test(src),
    'should create sums as Set');
});

test('getDates sorts sums array', () => {
  const src = readSource();
  assert.ok(/sums\.sort/.test(src),
    'should sort sums');
});
...
test('getDates uses stopwords for filtering', () => {
  ...
  assert.ok(/stopw\.has/.test(src),
    'should check words against stopwords');
});
...
test('getDates splits bigram words for stopword check', () => {
  ...
  assert.ok(/bis\[0\]/.test(src) && /bis\[1\]/.test(src),
    'should check both words');
});

test('getDates uses skip_bi Set', () => {
  const src = readSource();
  assert.ok(/skip_bi\s*=\s*new Set/.test(src),
    'should create skip_bi Set');
  assert.ok(/skip_bi\.add/.test(src),
    'should add to skip_bi');
  assert.ok(/skip_bi\.has/.test(src),
    'should check skip_bi');
});

test('getDates applies minimum count threshold', () => {
  const src = readSource();
  assert.ok(/min_n/.test(src),
    'should define min_n threshold');
  assert.ok(/sum\s*<\s*min_n/.test(src),
    'should compare sum against min_n');
});

test('getDates increments min_n when it equals 1', () => {
  const src = readSource();
  assert.ok(/min_n\s*===\s*1/.test(src) && /min_n\+\+/.test(src),
    'should increment min_n from 1 to 2');
});

test('getDates collects dates from non-skipped bigrams', () => {
  const src = readSource();
  assert.ok(/dates\s*=\s*new Set/.test(src),
    'should create dates Set');
  assert.ok(/dates\.add/.test(src),
    'should add dates');
});

test('getDates returns array of dates and skip_bi', () => {
  const src = readSource();
  assert.ok(/return\s*\[\s*dates\s*,\s*skip_bi\s*\]/.test(src),
    'should return [dates, skip_bi]');
});

test('getDates skip parameter controls threshold position', () => {
  const src = readSource();
  assert.ok(/if\s*\(\s*!\s*skip\s*\)/.test(src) || /!skip/.test(src),
    'should check !skip');
  assert.ok(/sum_pos\s*=\s*0/.test(src),
    'should reset sum_pos when skip=false');
});
...
test('source declares updateMentions method', () => {
  const src = readSource();
  assert.ok(/updateMentions\s*\(\s*data/.test(src),
    'should declare updateMentions(data) method');
});

test('updateMentions checks for data.bigrams existence', () => {
  const src = readSource();
  assert.ok(/!\s*data\.bigrams/.test(src),
    'should check for falsy data.bigrams');
});

test('updateMentions shows "No mentions" when no bigrams', () => {
  const src = readSource();
  assert.ok(/['"]<p>No mentions<\/p>['"]/.test(src) || /innerHTML\s*=.*No mentions/.test(src),
...
test('updateMentions calls getDates with skip=true first', () => {
  const src = readSource();
  assert.ok(/getDates\s*\(\s*data\s*,\s*true/.test(src),
    'should call getDates(data, true) first');
});

test('updateMentions retries getDates with skip=false if no dates', () => {
  const src = readSource();
  assert.ok(/dates\.size\s*===\s*0/.test(src) || /!dates\.size/.test(src),
    'should check dates size');
  assert.ok(/getDates\s*\(\s*data\s*,\s*false/.test(src),
    'should retry with getDates(data, false)');
});

test('updateMentions converts dates Set to sorted array', () => {
  const src = readSource();
  assert.ok(/Array\.from\s*\(\s*dates/.test(src),
    'should convert dates Set to array');
...
test('updateMentions aggregates dates within time windows', () => {
  const src = readSource();
  assert.ok(/dates_aggr/.test(src),
    'should define dates_aggr');
  assert.ok(/aggr\s*=\s*3600\s*\*\s*24\s*\*\s*1000/.test(src) || /aggr/.test(src),
...
test('updateMentions creates Date objects', () => {
  ...
  assert.ok(/new Date/.test(src),
    'should create Date objects');
  assert.ok(/\.setTime/.test(src),
    'should call setTime');
  assert.ok(/\.setMinutes/.test(src),
    'should call setMinutes');
  assert.ok(/\.setSeconds/.test(src),
    'should call setSeconds');
  assert.ok(/\.setMilliseconds/.test(src),
    'should call setMilliseconds');
});

test('updateMentions builds series data array', () => {
  const src = readSource();
  assert.ok(/series\s*=\s*\[\s*\]/.test(src),
    'should create series array');
  assert.ok(/series\.push/.test(src),
    'should push to series');
});

test('updateMentions adds rsstag_date to series items', () => {
  const src = readSource();
  assert.ok(/rsstag_date/.test(src),
    'should include rsstag_date property');
});

test('updateMentions calls renderChart with series', () => {
  const src = readSource();
  assert.ok(/this\.renderChart\s*\(\s*series/.test(src),
    'should call renderChart(series)');
});
...
test('renderChart creates bi_points object', () => {
  const src = readSource();
  assert.ok(/bi_points\s*=\s*\{\s*\}/.test(src),
    'should create bi_points object');
  assert.ok(/bi_points\s*\[\s*bi\s*\]/.test(src),
    'should access bi_points by bigram');
});
...
test('renderChart formats date labels with YYYY-MM-DD format', () => {
  ...
  assert.ok(/getDate/.test(src),
    'should use getDate');
  assert.ok(/prettyDate/.test(src),
    'should call prettyDate helper');
});

test('renderChart builds datasets from bi_points', () => {
  const src = readSource();
  assert.ok(/datasets\s*=\s*\[\s*\]/.test(src),
    'should create datasets array');
  assert.ok(/datasets\.push/.test(src),
    'should push to datasets');
});

test('renderChart dataset includes label with bigram name', () => {
  const src = readSource();
  assert.ok(/label\s*:/.test(src),
    'should set label');
  assert.ok(/label.*bi/.test(src),
    'should include bigram in label');
});

test('renderChart dataset includes sum in label', () => {
  const src = readSource();
  assert.ok(/\.reduce/.test(src),
    'should use reduce for sum');
  assert.ok(/acc\s*\+\s*v/.test(src) || /accumulator.*value/.test(src),
    'should sum values in reduce');
});

test('renderChart uses d3.interpolateCubehelixDefault for colors', () => {
  const src = readSource();
  assert.ok(/d3\.interpolateCubehelixDefault/.test(src),
    'should use d3.interpolateCubehelixDefault');
  assert.ok(/Math\.random/.test(src),
    'should use Math.random for color seed');
});

test('renderChart creates canvas element', () => {
  const src = readSource();
  assert.ok(/document\.createElement\s*\(\s*['"]canvas['"]/.test(src),
...
test('renderChart creates Chart instance', () => {
  ...
  assert.ok(/getContext\s*\(\s*['"]2d['"]/.test(src),
...
test('renderChart sets chart type to bar', () => {
  ...
  assert.ok(/type\s*:\s*['"]bar['"]/.test(src),
    'should set type to "bar"');
});

test('renderChart sets responsive to true', () => {
  const src = readSource();
  assert.ok(/responsive\s*:\s*true/.test(src),
    'should set responsive: true');
});

test('renderChart sets stacked: true on both axes', () => {
  const src = readSource();
  assert.ok(/stacked\s*:\s*true/.test(src),
    'should set stacked: true');
  assert.ok(/xAxes/.test(src),
    'should configure xAxes');
  assert.ok(/yAxes/.test(src),
    'should configure yAxes');
});

test('renderChart sets tooltip mode to index', () => {
  const src = readSource();
  assert.ok(/tooltips/.test(src),
    'should configure tooltips');
  assert.ok(/mode\s*:\s*['"]index['"]/.test(src),
    'should set mode to "index"');
  assert.ok(/intersect\s*:\s*false/.test(src),
    'should set intersect to false');
});
...
test('source declares renderLegend method', () => {
  const src = readSource();
  assert.ok(/renderLegend\s*\(\s*color/.test(src),
    'should declare renderLegend(color) method');
});

test('renderLegend iterates over color.domain()', () => {
  const src = readSource();
  assert.ok(/color\.domain/.test(src),
    'should call color.domain()');
});

test('renderLegend builds HTML string with inline styles', () => {
  const src = readSource();
  assert.ok(/html\s*\+=/.test(src),
    'should append to html string');
  assert.ok(/style/.test(src),
    'should include style attributes');
  assert.ok(/background-color/.test(src),
    'should set background-color');
  assert.ok(/color\s*\(\s*bi/.test(src),
    'should call color(bi) for background');
});
...
test('start method calls bindEvents', () => {
  const src = readSource();
  assert.ok(/start\s*\(\s*\)\s*\{[\s\S]*?this\.bindEvents/.test(src),
    'should call bindEvents in start');
});

test('bindEvents binds to BIGRAMS_MENTIONS_UPDATED event', () => {
  const src = readSource();
  assert.ok(/BIGRAMS_MENTIONS_UPDATED/.test(src),
    'should reference BIGRAMS_MENTIONS_UPDATED');
  assert.ok(/this\.ES\.bind\s*\(\s*this\.ES\.BIGRAMS_MENTIONS_UPDATED/.test(src),
...
test('source uses d3 library', () => {
  const src = readSource();
  assert.ok(/d3\./.test(src),
    'should use d3 library');
});
