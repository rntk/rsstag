...
test('source has named export TagsClustersTxtList', () => {
  const src = readSource();
  assert.ok(/export class TagsClustersTxtList extends React\.Component/.test(src),
    'should export TagsClustersTxtList class');
});

test('source has default export TagsClustersList', () => {
  const src = readSource();
  assert.ok(/export default class TagsClustersList extends React\.Component/.test(src),
...
test('TagsClustersTxtList declares updateTags method', () => {
  const src = readSource();
  assert.ok(/updateTags\s*\(\s*state\s*\)/.test(src),
    'should declare updateTags(state) method');
});

test('TagsClustersTxtList updateTags calls setState', () => {
  const src = readSource();
  assert.ok(/this\.setState\s*\(\s*state\s*\)/.test(src),
    'should call setState with state');
});
...
test('TagsClustersTxtList componentDidMount binds TAGS_CLUSTERS_UPDATED', () => {
  const src = readSource();
  assert.ok(/TAGS_CLUSTERS_UPDATED/.test(src),
    'should reference TAGS_CLUSTERS_UPDATED');
  assert.ok(/this\.props\.ES\.bind\s*\(\s*this\.props\.ES\.TAGS_CLUSTERS_UPDATED/.test(src),
...
test('TagsClustersTxtList componentWillUnmount unbinds TAGS_CLUSTERS_UPDATED', () => {
  const src = readSource();
  assert.ok(/this\.props\.ES\.unbind\s*\(\s*this\.props\.ES\.TAGS_CLUSTERS_UPDATED/.test(src),
...
test('TagsClustersTxtList render checks state and clusters', () => {
  const src = readSource();
  assert.ok(/this\.state && this\.state\.clusters/.test(src),
...
test('TagsClustersList constructor accepts props', () => {
  ...
  assert.equal(matches && matches.length, 2,
    'both classes should have constructor(props)');
});
...
test('TagsClustersList constructor binds updateTags', () => {
  ...
  assert.equal(matches && matches.length, 2,
    'both classes should bind updateTags');
});
...
test('TagsClustersList declares componentDidMount', () => {
  ...
  assert.equal(matches && matches.length, 2,
    'both classes should declare componentDidMount');
});
...
test('TagsClustersList declares componentWillUnmount', () => {
  ...
  assert.equal(matches && matches.length, 2,
    'both classes should declare componentWillUnmount');
});

test('TagsClustersList render returns "No clusters" paragraph when empty', () => {
  const src = readSource();
  assert.ok(/<p>No clusters<\/p>/.test(src),
    'should return <p>No clusters</p> when no clusters');
});

test('TagsClustersList render returns div with cloud class', () => {
  const src = readSource();
  assert.ok(/<div className\s*=\s*['"]cloud['"]/.test(src),
    'should render div with cloud class');
});

test('TagsClustersList render sorts clusters by post count descending', () => {
  const src = readSource();
  assert.ok(/\.sort\s*\(/.test(src),
    'should sort clusters');
  assert.ok(/b\.pids\.length\s*-\s*a\.pids\.length/.test(src),
    'should sort by pids length descending');
});

test('TagsClustersList render creates links with cluster_link class', () => {
  const src = readSource();
  assert.ok(/cluster_link/.test(src),
    'should use cluster_link CSS class');
});

test('TagsClustersList render links to /posts/ with joined pids', () => {
  const src = readSource();
  assert.ok(/href=\{\s*[`'"]\/posts\/\$\{/.test(src) || /href=\{\s*['"]\/posts\/['"]\s*\+/.test(src),
    'should link to /posts/ with pids');
});

test('TagsClustersList render shows label and count in parentheses', () => {
  const src = readSource();
  assert.ok(/\$\{pids_l\.pids\.length\}/.test(src) || /pids\.length/.test(src),
