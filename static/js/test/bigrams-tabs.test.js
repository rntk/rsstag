...
test('source defines TAB_CLOUD constant', () => {
  ...
  assert.ok(/TAB_CLOUD\s*=\s*['"]cloud['"]/.test(src), 'should define TAB_CLOUD = "cloud"');
});

test('source defines TAB_TABLE constant', () => {
  const src = readSource();
  assert.ok(/TAB_TABLE\s*=\s*['"]table['"]/.test(src), 'should define TAB_TABLE = "table"');
});
...
test('source exports named BiGramsTabs class extending React.Component', () => {
  const src = readSource();
  assert.ok(
    /export class BiGramsTabs extends React\.Component/.test(src),
...
test('constructor sets cloud tab', () => {
  const src = readSource();
  assert.ok(/this\.tabs\.set\s*\(\s*TAB_CLOUD/.test(src), 'should set cloud tab');
});

test('cloud tab title is "Tags Cloud"', () => {
  const src = readSource();
  assert.ok(
    /TAB_CLOUD.*['"]Tags Cloud['"]/.test(src),
...
test('constructor sets table tab', () => {
  const src = readSource();
  assert.ok(/this\.tabs\.set\s*\(\s*TAB_TABLE/.test(src), 'should set table tab');
});

test('table tab title is "Tags Table"', () => {
  const src = readSource();
  assert.ok(
    /TAB_TABLE.*['"]Tags Table['"]/.test(src),
...
test('constructor sets current tab to cloud', () => {
  const src = readSource();
  assert.ok(
    /this\.state\s*=\s*\{[\s\S]*?current\s*:\s*TAB_CLOUD/.test(src),
...
test('changeTab updates current in state', () => {
  const src = readSource();
  assert.ok(/this\.setState\s*\(\s*\{\s*current\s*:/.test(src), 'should set state.current');
});

test('source declares updateVisibility method', () => {
  const src = readSource();
  assert.ok(/updateVisibility\s*\(\s*\)/.test(src), 'should declare updateVisibility() method');
});

test('updateVisibility queries tags_page element', () => {
  const src = readSource();
  assert.ok(
    /getElementById\s*\(\s*['"]tags_page['"]/.test(src),
...
test('updateVisibility queries bigrams_table_page element', () => {
  const src = readSource();
  assert.ok(
    /getElementById\s*\(\s*['"]bigrams_table_page['"]/.test(src),
...
test('updateVisibility shows tags page when cloud is active', () => {
  const src = readSource();
  assert.ok(
    /tagsPage\.style\.display\s*=\s*['"]block['"]/.test(src),
...
test('updateVisibility hides table page when cloud is active', () => {
  const src = readSource();
  assert.ok(
    /bigramsTablePage\.style\.display\s*=\s*['"]none['"]/.test(src),
...
test('updateVisibility hides tags page when table is active', () => {
  const src = readSource();
  assert.ok(/display\s*=\s*['"]none['"]/.test(src), 'should set display to "none"');
});

test('updateVisibility shows table page when table is active', () => {
  const src = readSource();
  assert.ok(
    /bigramsTablePage\.style\.display\s*=\s*['"]block['"]/.test(src),
...
test('updateVisibility checks both elements exist before modifying', () => {
  const src = readSource();
  assert.ok(/if\s*\(\s*tagsPage && bigramsTablePage/.test(src), 'should check both elements exist');
});
...
test('componentDidMount calls updateVisibility', () => {
  const src = readSource();
  assert.ok(
    /componentDidMount[\s\S]*?updateVisibility/.test(src),
...
test('componentDidUpdate calls updateVisibility', () => {
  const src = readSource();
  assert.ok(
    /componentDidUpdate[\s\S]*?updateVisibility/.test(src),
...
test('render returns div with bigrams_tabs_container class', () => {
  const src = readSource();
  assert.ok(/bigrams_tabs_container/.test(src), 'should use bigrams_tabs_container CSS class');
});
...
test('render maps tabs to li elements', () => {
  const src = readSource();
  assert.ok(
    /for\s*\(\s*let\s+\[.*name.*title.*\]\s+of\s+this\.tabs/.test(src) || /this\.tabs/.test(src),
...
test('render tab has data-tab attribute', () => {
  const src = readSource();
  assert.ok(/data-tab\s*=\s*\{?\s*name/.test(src), 'should set data-tab attribute from tab name');
});
...
test('render displays tab title text', () => {
  const src = readSource();
  assert.ok(/\{?\s*title\s*\}/.test(src), 'should display tab title');
});

test('render tab key includes tab_ prefix', () => {
  const src = readSource();
  assert.ok(/['"]tab_['"]/.test(src) || /tab_\s*\+/.test(src), 'should use "tab_" prefix for keys');
});
...
test('source uses document.getElementById', () => {
  const src = readSource();
  assert.ok(/document\.getElementById/.test(src), 'should use document.getElementById');
});
