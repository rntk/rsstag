...
test('class name is TagMentionsChart', () => {
  const src = readSource();
  assert.ok(
    /export default class TagMentionsChart/.test(src),
...
test('source declares updateMentions method', () => {
  const src = readSource();
  assert.ok(
    /updateMentions\s*\(\s*data\s*\)/.test(src),
...
test('updateMentions checks data.dates.length for empty data', () => {
  const src = readSource();
  assert.ok(/!data\.dates\.length/.test(src), 'should check !data.dates.length');
});

test('updateMentions calls hideChart when no dates', () => {
  const src = readSource();
  assert.ok(/this\.hideChart\(\)/.test(src), 'should call hideChart() when no dates');
});

test('updateMentions returns early when no dates', () => {
  const src = readSource();
  assert.ok(/this\.hideChart\(\);\s*return;/.test(src), 'should return after hideChart');
});

test('updateMentions creates an aggregation map (mp)', () => {
  const src = readSource();
  assert.ok(/let mp\s*=\s*\{\}/.test(src), 'should create empty mp object');
});
...
test('updateMentions creates a slice copy of dates', () => {
  const src = readSource();
  assert.ok(/data\.dates\.slice\(\)/.test(src), 'should use slice() to copy dates');
});

test('updateMentions uses aggregation window of 86400000ms (24h)', () => {
  const src = readSource();
  assert.ok(/3600\s*\*\s*24\s*\*\s*1000/.test(src), 'should define 24h aggregation window');
});

test('updateMentions multiplies date timestamps by 1000', () => {
  const src = readSource();
  assert.ok(/u\s*\*=\s*1000/.test(src), 'should convert seconds to milliseconds');
});
...
test('updateMentions uses prettyDate for zero-padded month and day', () => {
  const src = readSource();
  assert.ok(/this\.prettyDate/.test(src), 'should call this.prettyDate');
});

test('updateMentions increments count per date bucket', () => {
  const src = readSource();
  assert.ok(
    /mp\[k\]\+\+/.test(src) || /mp\[k\]\s*=\s*mp\[k\]\s*\+\s*1/.test(src),
...
test('updateMentions builds labels and values arrays from mp', () => {
  const src = readSource();
  assert.ok(/let labels\s*=\s*\[\]/.test(src), 'should create labels array');
  assert.ok(/let values\s*=\s*\[\]/.test(src), 'should create values array');
  assert.ok(/labels\.push/.test(src), 'should push to labels');
  assert.ok(/values\.push/.test(src), 'should push to values');
});

test('updateMentions calls renderChart with tag, labels, values', () => {
  const src = readSource();
  assert.ok(
    /this\.renderChart\s*\(\s*tag\s*,\s*labels\s*,\s*values\s*\)/.test(src),
...
test('source declares prettyDate method', () => {
  const src = readSource();
  assert.ok(/prettyDate\s*\(\s*n\s*\)/.test(src), 'should declare prettyDate(n) method');
});

test('prettyDate treats 0 as 1', () => {
  const src = readSource();
  assert.ok(
    /if\s*\(\s*n\s*===\s*0\s*\)\s*\{?\s*\n?\s*n\s*=\s*1/.test(src),
...
test('prettyDate adds leading zero for single-digit numbers', () => {
  const src = readSource();
  assert.ok(/if\s*\(\s*n\s*<\s*10\s*\)/.test(src), 'should check n < 10');
  assert.ok(/zero\s*=\s*['"]0['"]/.test(src), 'should set zero to "0"');
});

test('prettyDate returns zero-padded string', () => {
  const src = readSource();
  assert.ok(/return zero\s*\+\s*n/.test(src), 'should return zero + n');
});
...
test('source declares renderChart method', () => {
  const src = readSource();
  assert.ok(
    /renderChart\s*\(\s*tag\s*,\s*labels\s*,\s*values\s*\)/.test(src),
...
test('renderChart creates a dataset object', () => {
  const src = readSource();
  assert.ok(/label\s*:\s*tag/.test(src), 'should set dataset label to tag');
  assert.ok(/data\s*:\s*values/.test(src), 'should set dataset data to values');
});

test('renderChart creates dataset with labels array', () => {
  const src = readSource();
  assert.ok(/labels\s*:\s*labels/.test(src), 'should use labels in dataset');
});
...
test('renderChart creates a canvas element', () => {
  const src = readSource();
  assert.ok(
    /document\.createElement\s*\(\s*['"]canvas['"]\s*\)/.test(src),
...
test('renderChart creates Chart with bar type', () => {
  const src = readSource();
  assert.ok(/new Chart/.test(src), 'should create new Chart instance');
  assert.ok(/type\s*:\s*['"]bar['"]/.test(src), 'should set chart type to bar');
});

test('renderChart uses canvas 2d context', () => {
  const src = readSource();
  assert.ok(/getContext\s*\(\s*['"]2d['"]\s*\)/.test(src), 'should get 2d context');
});
...
test('source declares hideChart method', () => {
  const src = readSource();
  assert.ok(/hideChart\s*\(\s*\)/.test(src), 'should declare hideChart() method');
});
...
test('bindEvents binds TAG_MENTIONS_UPDATED event', () => {
  const src = readSource();
  assert.ok(/TAG_MENTIONS_UPDATED/.test(src), 'should reference TAG_MENTIONS_UPDATED');
  assert.ok(
    /this\.ES\.bind\s*\(\s*this\.ES\.TAG_MENTIONS_UPDATED/.test(src),
...
test('bindEvents binds updateMentions as handler', () => {
  const src = readSource();
  assert.ok(
    /this\.ES\.bind\s*\([^,]+,\s*this\.updateMentions\s*\)/.test(src),
...
test('source uses Date constructor', () => {
  const src = readSource();
  assert.ok(/new Date\(\)/.test(src), 'should use new Date()');
});
