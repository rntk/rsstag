...
test('source exports a default class', () => {
  ...
  assert.ok(/export default class TopicsTexts/.test(src),
...
test('constructor binds updateData method', () => {
  const src = readSource();
  assert.ok(/this\.updateData\s*=\s*this\.updateData\.bind\s*\(\s*this\s*\)/.test(src),
...
test('source declares updateData method', () => {
  const src = readSource();
  assert.ok(/updateData\s*\(\s*data\s*\)\s*\{/.test(src),
    'should declare updateData(data) method');
});

test('updateData uses a window of 5 for context', () => {
  const src = readSource();
  assert.ok(/const window\s*=\s*5/.test(src),
    'should define window = 5 for context');
});

test('updateData iterates over data.topics', () => {
  const src = readSource();
  assert.ok(/for\s*\(\s*let topic of data\.topics/.test(src),
    'should iterate over data.topics');
});
...
test('updateData splits text by spaces', () => {
  const src = readSource();
  assert.ok(/txt\.split\s*\(\s*['"]\s+['"]\s*\)/.test(src),
    'should split text by spaces');
});

test('updateData calculates st_pos with Math.max', () => {
  const src = readSource();
  assert.ok(/Math\.max\s*\(\s*i\s*-\s*window\s*,\s*0\s*\)/.test(src),
    'should use Math.max for start position');
});

test('updateData handles end_pos boundary check', () => {
  const src = readSource();
  assert.ok(/end_pos\s*>\s*words\.length/.test(src),
    'should check end_pos against words.length');
});

test('updateData calls renderWordtree for each topic', () => {
  const src = readSource();
  assert.ok(/this\.renderWordtree\s*\(\s*topic\s*,\s*texts\s*\)/.test(src),
...
test('source declares renderWordtree method', () => {
  const src = readSource();
  assert.ok(/renderWordtree\s*\(\s*topic\s*,\s*topic_texts\s*\)\s*\{/.test(src),
...
test('renderWordtree appends div to container', () => {
  const src = readSource();
  assert.ok(/this\._container\.appendChild\s*\(\s*div\s*\)/.test(src),
...
test('renderWordtree creates WordTree chart', () => {
  const src = readSource();
  assert.ok(/new google\.visualization\.WordTree/.test(src),
...
test('renderWordtree sets format to implicit', () => {
  ...
  assert.ok(/format\s*:\s*['"]implicit['"]/.test(src),
    'should set wordtree format to implicit');
});

test('renderWordtree sets word option to topic', () => {
  const src = readSource();
  assert.ok(/word\s*:\s*topic/.test(src),
    'should set word option to topic');
});

test('renderWordtree sets type to double', () => {
  const src = readSource();
  assert.ok(/type\s*:\s*['"]double['"]/.test(src),
    'should set wordtree type to double');
});
...
test('bindEvents binds TOPICS_TEXTS_UPDATED event', () => {
  const src = readSource();
  assert.ok(/this\.ES\.bind\s*\(\s*this\.ES\.TOPICS_TEXTS_UPDATED\s*,\s*this\.updateData/.test(src),
...
test('source has no React import', () => {
  ...
  assert.ok(!/import.*React/.test(src),
    'should not import React (vanilla JS)');
});
...
test('renderWordtree uses arrayToDataTable', () => {
  const src = readSource();
  assert.ok(/google\.visualization\.arrayToDataTable/.test(src),
