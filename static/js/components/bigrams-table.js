...
export default class BigramsTable extends React.Component {
  ...
  processBigrams() {
    ...
    let firstWords = Object.keys(firstWordCounts).sort(
      (a, b) => firstWordCounts[b] - firstWordCounts[a]
    );
    let secondWords = Object.keys(secondWordCounts).sort(
      (a, b) => secondWordCounts[b] - secondWordCounts[a]
    );
    ...
    for (let iter = 0; iter < iterations; iter++) {
      ...
      firstWords = firstWords.sort((a, b) => {
        const aTopConnections = [...(firstWordConnections[a] || [])].filter((w) =>
          topSecondWords.has(w)
        ).length;
        const bTopConnections = [...(firstWordConnections[b] || [])].filter((w) =>
          topSecondWords.has(w)
        ).length;
      ...
      secondWords = secondWords.sort((a, b) => {
        const aTopConnections = [...(secondWordConnections[a] || [])].filter((w) =>
          topFirstWords.has(w)
        ).length;
        const bTopConnections = [...(secondWordConnections[b] || [])].filter((w) =>
          topFirstWords.has(w)
        ).length;
  ...
  renderCircle(cellData, maxCount) {
    ...
    return (
      <a
        href={cellData.url}
        className="bigram-cell-link"
        title={`${cellData.tag}: ${cellData.count}`}
      >
          ...
          <div className="legend-items">
            <span className="legend-item">
              <span
                ...
                style={{ backgroundColor: '#3498db', width: 20, height: 20 }}
              ></span>
              High
            </span>
            <span className="legend-item">
              <span
                className="legend-circle"
                style={{ backgroundColor: '#e67e22', width: 15, height: 15 }}
              ></span>
              Medium
            </span>
            <span className="legend-item">
              <span
                className="legend-circle"
                style={{ backgroundColor: '#f1c40f', width: 10, height: 10 }}
              ></span>
              Low-Medium
            </span>
            <span className="legend-item">
              <span
                className="legend-circle"
                style={{ backgroundColor: '#e74c3c', width: 6, height: 6 }}
              ></span>
              Low
            </span>
          </div>
