    ...
    return (
      <tr className="sentence-row">
        ...
        <td className="mid sentence-cell">
          <div className="sentence-cell-inner">
            {context.post_url ? (
              <a href={context.post_url} target="_blank" rel="noopener noreferrer">
                {context.mid}
              </a>
            ) : (
              context.mid
...
export default class SentenceTree extends React.Component {
  constructor(props) {
    ...
    this.state = {
      ...
      cluster_links:
        source.cluster_links && typeof source.cluster_links === 'object'
          ? source.cluster_links
          : {},
    };
  }
  ...
  componentDidMount() {
    ...
    if (!this.props.s_tree_data && window.s_tree_data) {
      ...
      if (
        this.state.tag !== (source.tag || '') ||
        JSON.stringify(this.state.words) !==
          JSON.stringify(Array.isArray(source.words) ? source.words : []) ||
        JSON.stringify(this.state.clusters) !==
          JSON.stringify(
            source.clusters && typeof source.clusters === 'object' ? source.clusters : {}
          )
      ) {
        this.setState({
          tag: source.tag || '',
