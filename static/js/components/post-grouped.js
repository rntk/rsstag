...
export default class PostGroupedPage {
  ...
  getSelectionsForTopic(topicName) {
    ...
    sentenceNumbers.forEach((globalSentenceNumber) => {
      ...
      if (
        !sentenceMeta ||
        !sentenceMeta.postId ||
        !Number.isFinite(sentenceMeta.postSentenceNumber)
      ) {
        return;
      }
  ...
  highlightSentences(sentenceIndices, color, focusIndex, shouldScroll = true) {
    ...
    if (highlightedElements.length > 0) {
      const targetIdx = Number.isFinite(focusIndex)
        ? Math.max(0, Math.min(highlightedElements.length - 1, focusIndex))
        : 0;
      ...
      const targetSpan =
        targetSentence &&
        highlightedBySentence[targetSentence] &&
  ...
  initLocalTabs() {
    ...
    localTabs.forEach((tab) => {
      tab.addEventListener('click', () => {
        ...
        // Toggle contents
        postSection
          .querySelectorAll('.local-tab-content')
          .forEach((c) => c.classList.remove('active'));
        const targetContent = postSection.querySelector(
          `.local-tab-content[data-local-content="${target}"]`
        );
        if (targetContent) {
