...
export default class TopicsRiverChart {
  ...
  _calculateEffectiveLength() {
    ...
    const validArticleLength =
      typeof this.articleLength === 'number' && this.articleLength > 0
        ? this.articleLength
        : Infinity;
  ...
  init() {
    ...
    if (typeof d3 === 'undefined') {
      this.container.innerHTML =
        '<div style="color: red; padding: 20px;">Error: D3.js is not loaded.</div>';
      return;
    }
    this.container.innerHTML = '';
  ...
  _updateStyles() {
    ...
    if (this.activeTopic) {
      ...
      paths.transition().duration(200).style('opacity', 0.85).style('stroke', 'none');
