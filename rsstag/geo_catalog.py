...
class RssTagGeoCatalog:
        ...
        for coll_name in self._collections:
            for indx in indexes:
                try:
                    ...
                    self._log.warning('Can`t create index "%s" on "%s". May be alredy exists. Info: e', coll_name, indx, e)
    ...
    def get_country_by_name(self, country_name: str) -> Optional[dict]:
        try:
            result = self._db.countries.find_one({
            ...
            result = None
            self._log.error('Can`t get country info by name %s. Info: %s', country_name, e)
