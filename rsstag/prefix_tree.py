...
class PrefixTreeBuilder:
    ...
    def get_top_n(self, n: int) -> list[tuple[str, int]]:
        ...
        def collect_prefix_counts(
            node: dict, prefix: str = "", length: int = n
        ) -> list[tuple[str, int]]:
            ...
            for char, child in node.items():
                if char != "count":
                    results.extend(
                        collect_prefix_counts(child["children"], prefix + char, length)
                    )
        ...
        def build_compact_subtree(node: dict, name: str = "") -> dict:
            ...
            for char, child in child_items:
                ...
                if is_single and path:
                    ...
                    end_node, count = self._get_single_path_node(
                        child["children"], path
                    )
