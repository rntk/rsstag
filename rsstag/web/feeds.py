...
def on_delete_feeds_categories_post(app, user: dict, request: Request) -> Response:
    try:
        ...
        logging.info("Delete request for user %s: feeds=%s, categories=%s", user["sid"], feed_ids, category_ids)
        
        if not feed_ids and not category_ids:
            return Response(
                json.dumps({"status": "error", "message": "No feeds or categories selected"}),
        ...
        if category_ids:
            db_feeds = app.feeds.get_by_categories(user["sid"], category_ids, projection={"feed_id": True})
            for f in db_feeds:
        ...
        if feed_ids:
            ...
            return Response(
             ...
             return Response(
                json.dumps({"status": "error", "message": "No feeds found for selection"}),
                mimetype="application/json",
                status=404
            )

    except Exception as e:
        logging.error("Failed to process delete request for user %s: %s", user["sid"], e)
        return Response(
